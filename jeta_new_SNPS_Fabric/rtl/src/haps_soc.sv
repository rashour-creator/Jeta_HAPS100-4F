...
`endif
    ...
    */
    ...
    axi_mm #(.DWDT(32),.AWDT(32),.IDWDT(7))axi_bram_ctrl        (.clk(core_clk), .resetn(~reset_sync)); // i_SNPS_fabric -> i_axi_mmio
    axi_mm #(.DWDT(32),.AWDT(32),.IDWDT(7))axi_uart_mm          (.clk(core_clk), .resetn(~reset_sync)); // i_SNPS_fabric -> i_axi_mmio
    axi_mm #(.DWDT(64),.AWDT(32),.IDWDT(10))axi_slave_transactor(.clk(core_clk), .resetn(~reset_sync)); // i_SNPS_fabric -> i_axi_slave_xtor
...
*/
   ...
   `undef CAP
    ...
    logic dut_i_axi_a2x_2_busy_status;
    ...
    i_SNPS_fabric
        ...
        `AXI_MM(axi_transactor_slav03,axi_slave_transactor),
        ...
        .i_axi_a2x_2_busy_status        (dut_i_axi_a2x_2_busy_status)
    );
    ...
    `define CAP 1
    axi_slave_xtor #(
        .S1_DATA_WIDTH(axi_slave_transactor.DWDT),
        .S1_ADDR_WIDTH(axi_slave_transactor.AWDT),
        .S1_ID_WIDTH(axi_slave_transactor.IDWDT)
    ) i_axi_slave_xtor
    (
         //I_axi_slave (axi_slave) signal_prefix='S1_'
        .S1_ACLK                  (core_clk         ),
        .S1_ARESETn               ( reset_n         ),    // reset_n or ~reset_sync (as it requires active low reset)
        // Axi port
        `AXI_MM(S1,axi_slave_transactor),
        .S1_CSYSREQ               (1'b0             ),
    ...
    `ifdef ENABLE_DEBUG
        initial begin: IICE_core_clk
            ...
            `AXI_DEBUG(axi_slave_transactor);
        end
