...
//              |---------| 
// **************************************************************************************
module i_axi_a2x_DW_axi_a2x_sp_add_split (/*AUTOARG*/
   // Outputs
  ...
  wire   [`i_axi_a2x_A2X_IDW-1:0]                      id_i;  
  ...
  wire   [`i_axi_a2x_A2X_BTW-1:0]                      ws_burst;         // Wrap Splitter output Burst
  ...
  // Burst Type Decode
  assign  wrap     = (burst_i==`i_axi_a2x_ABURST_WRAP)? 1'b1 : 1'b0;

  // AXI Fixed Burst 
  assign  ws_fixed = (A2X_PP_MODE==0)? 1'b0: (burst_i==`i_axi_a2x_ABURST_FIXED)? 1'b1 : 1'b0;
  ...
  if (A2X_PP_MODE==0) begin: ONEK_BLK
    ...
    i_axi_a2x_DW_axi_a2x_sp_add_onek
     #(
  ...
  if (BYPASS_WS==0) begin: WSBLK
    wire   [`i_axi_a2x_A2X_BTW-1:0]                      ws_burst_w;
    wire                                       alast_as_w;
    wire                                       active_as_w;
    wire                                       second_burst_w;
    wire                                       split_w;
    ...
    i_axi_a2x_DW_axi_a2x_sp_add_ws
     #(
    ...
    // AXI Burst type
    assign ws_burst_w   = (wrap | second_burst)? `i_axi_a2x_ABURST_INCR: burst_i; 
