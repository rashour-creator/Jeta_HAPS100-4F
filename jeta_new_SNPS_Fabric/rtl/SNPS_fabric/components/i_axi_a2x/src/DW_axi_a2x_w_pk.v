...
//
//*********************************************************
module i_axi_a2x_DW_axi_a2x_w_pk (/*AUTOARG*/
   // Outputs
  ...
  always @(posedge clk or negedge resetn) begin: wrap_cnt_PROC
    if (resetn == 1'b0) begin
      ...
      if (load)
        wrap_cnt <= 8'd0;
      else if ((burst_i==`i_axi_a2x_ABURST_WRAP) && wready_i && wvalid_i)
        wrap_cnt <= wrap_cnt + 8'd1;
    end
  end
    
  // Wrap Boundary Reached.
  assign wrap_b = (burst_i==`i_axi_a2x_ABURST_WRAP) && (wrap_cnt==wrap_len);
  ...
  always @(posedge clk or negedge resetn) begin: shift_PROC
    if (resetn == 1'b0) begin
      ...
      if ( wready_i && wvalid_i) begin
        if (burst_i==`i_axi_a2x_ABURST_FIXED && A2X_PP_MODE==1)
  ...
  assign w_fifo_push   = (shift_msb | wlast_i | pp_subsized | (!resize_i) | wrap_b | (burst_i==`i_axi_a2x_ABURST_FIXED)) & wvalid_i & wready_i;
