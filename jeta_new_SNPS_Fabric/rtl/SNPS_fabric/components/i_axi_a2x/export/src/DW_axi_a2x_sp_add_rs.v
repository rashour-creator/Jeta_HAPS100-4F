...
// Resize Address
// **************************************************************************************
module i_axi_a2x_DW_axi_a2x_sp_add_rs (/*AUTOARG*/
   // Outputs
   pyld_o, 
   rs_ratio,
   ds_fixed_decomp,
  ...
  // Signal Decelaration
  // **************************************************************************************
  wire   [`i_axi_a2x_A2X_IDW-1:0]                      id_i;            // Payload Decode
  wire   [A2X_AW-1:0]                        addr_i; 
  wire   [BLW_AS-1:0]                        len_i; 
  wire   [`i_axi_a2x_A2X_BSW-1:0]                      size_i;     
  wire   [`i_axi_a2x_A2X_BTW-1:0]                      burst_i;   
  wire   [`i_axi_a2x_A2X_LTW-1:0]                      lock_i;   
  wire   [`i_axi_a2x_A2X_CTW-1:0]                      cache_i; 
  wire   [`i_axi_a2x_A2X_PTW-1:0]                      prot_i; 
  ...
  wire   [`i_axi_a2x_A2X_RSW-1:0]                      resize_i;
  wire                                       hburst_type;

  wire   [BLW_RS:0]                          len_p1;          // Length plus 1

  wire   [`i_axi_a2x_A2X_BSW-1:0]                      ds_size;         // Downsized Size
  wire   [`i_axi_a2x_A2X_BSW-1:0]                      us_size;         // Upsized size
  wire   [`i_axi_a2x_A2X_BSW-1:0]                      rs_size;         // Resized Size
  ...
  wire   [`i_axi_a2x_A2X_BTW-1:0]                      rs_burst;   
  ...
  // spyglass enable_block TA_09
    ...
    assign ds_fixed_decomp = (burst_i==`i_axi_a2x_ABURST_FIXED) && (size_i>A2X_SP_MAX_SIZE);
   // assign ds_fixed_len    = ((pp_bytes_beat-ds_addr_sub[5:0])>>ds_size)-1;
    ...
    assign rs_burst    = ((size_i>A2X_SP_MAX_SIZE) && (burst_i==`i_axi_a2x_ABURST_FIXED))? `i_axi_a2x_ABURST_INCR : burst_i;
  ...
  if (A2X_UPSIZE==1) begin: USBLK
    ...
    // **************************************************************************************
    assign us_size = ((size_i==A2X_PP_MAX_SIZE) && resize_i && (burst_i!=`i_axi_a2x_ABURST_FIXED))? A2X_SP_MAX_SIZE : size_i;
    ...
    always @(*) begin: us_len_PROC
      ...
      if ((size_i==A2X_PP_MAX_SIZE) && resize_i && (burst_i!=`i_axi_a2x_ABURST_FIXED)) begin
