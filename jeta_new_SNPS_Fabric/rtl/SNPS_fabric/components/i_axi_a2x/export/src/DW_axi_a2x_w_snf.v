...
module i_axi_a2x_DW_axi_a2x_w_snf (/*AUTOARG*/
   ...
   wlast_pp, rs_pyld_i, pk_w_wb_len
    ...
    wire   [`i_axi_a2x_A2X_BSW-1:0]                        size;     
    wire   [`i_axi_a2x_A2X_BTW-1:0]                        burst;     
    wire                                         resize;
    wire                                         wrap_ds; 
    wire                                         wrap_us; 
    wire                                         wlast_int; 
   ...
   //*************************************************************************************
    ...
    assign wrap_us = (A2X_UPSIZE==1)? (burst==`i_axi_a2x_ABURST_WRAP) : 1'b0;
    assign wrap_ds = (A2X_DOWNSIZE==1)? (burst==`i_axi_a2x_ABURST_WRAP) : 1'b0;
   ...
   always @(*) begin: snf_rs_PROC
     ...
     if ((A2X_UPSIZE==1) && ((size==A2X_PP_MAX_SIZE) && resize)) begin
       ...
       if (burst==`i_axi_a2x_ABURST_FIXED)
  ...
  if ((A2X_DOWNSIZE==1) || (A2X_UPSIZE==1)) begin: RS_SNF
    always @(posedge clk_pp or negedge resetn_pp) begin: wrap_cnt_PROC
      if (resetn_pp == 1'b0) begin 
        ...
        if (siu_buf_mode==`i_axi_a2x_SNF_MODE) begin
    ...
    // SJ : This is not a functional issue, this is as per the requirement.
    //      Hence this can be waived.  
    assign wrap_max_cnt   = (siu_buf_mode==`i_axi_a2x_CT_MODE)? 1'b0 : (wrap_ds)? (wrap_cnt==ds_wb_len) : (wrap_us)? (wrap_cnt==pk_w_wb_len) : 1'b0; 
  ...
  // and W Channel valid. 
  //*************************************************************************************
  assign snf_push_n = (siu_buf_mode==`i_axi_a2x_SNF_MODE)? !(wready_pp & wvalid_pp & wlast_int) : 1'b1;
