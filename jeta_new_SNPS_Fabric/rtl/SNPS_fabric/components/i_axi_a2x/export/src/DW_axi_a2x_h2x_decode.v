...
module i_axi_a2x_DW_axi_a2x_h2x_decode (/*AUTOARG*/
  ...
  output  [`i_axi_a2x_A2X_HBLW-1:0]                     hburst_dp;
  output                                      hwrite_dp;
  output  [2:0]                               hsize_dp;        // AHB size              
  output  [`i_axi_a2x_A2X_IDW-1:0]                      hmaster_dp;
  output                                      hmastlock_dp;

  //*************************************************************************************
  // I/O Decelaration
  //*************************************************************************************
  wire    [`i_axi_a2x_A2X_IDW-1:0]                      id;
  ...
  reg     [1:0]                               htrans_dp;   
  reg     [`i_axi_a2x_A2X_HBLW-1:0]                     hburst_dp; 
  reg                                         hwrite_dp;
  reg     [2:0]                               hsize_dp;
  reg                                         hmastlock_dp;
  reg     [`i_axi_a2x_A2X_IDW-1:0]                      hmaster_dp;
  ...
  assign   burst       = ((hburst==`i_axi_a2x_HBURST_WRAP4)||(hburst==`i_axi_a2x_HBURST_WRAP8)|| (hburst==`i_axi_a2x_HBURST_WRAP16))? `i_axi_a2x_AWRAP: `i_axi_a2x_AINCR;

  assign   burst_type  = (hburst==`i_axi_a2x_HBURST_INCR)? 1'b1: 1'b0;
  ...
  always@(*) begin: alen_PROC
    ...
    case(hburst)
      ...
      `i_axi_a2x_HBURST_WRAP4:  alen = 4'h3;
      `i_axi_a2x_HBURST_INCR4:  alen = 4'h3;
      `i_axi_a2x_HBURST_WRAP8:  alen = 4'h7;
      `i_axi_a2x_HBURST_INCR8:  alen = 4'h7;
      `i_axi_a2x_HBURST_WRAP16: alen = 4'hf;
      `i_axi_a2x_HBURST_INCR16: alen = 4'hf;
      `i_axi_a2x_HBURST_INCR:   alen = (hwrite)? hincr_wbcnt : hincr_rbcnt; 
  ...
  // AXI Unlock Payload
  assign   haw_unlk_pyld   = {1'b0, haddr_sb, id, addr, 1'b0, {A2X_BLW{1'b0}}, 3'h0, `i_axi_a2x_AINCR, 2'b00, cache, prot};
  ...
  always @(posedge clk or negedge resetn) begin: dp_PROC
    if (resetn==1'b0) begin
      htrans_dp    <= 2'b0; 
      hburst_dp    <= {`i_axi_a2x_A2X_HBLW{1'b0}}; 
      hwrite_dp    <= 1'b0;
      hsize_dp     <= 3'b0;
      hmaster_dp   <= {`i_axi_a2x_A2X_IDW{1'b0}};
      hmastlock_dp <= 1'b0;
    end else begin
      if(hready) begin
        htrans_dp    <= htrans;
