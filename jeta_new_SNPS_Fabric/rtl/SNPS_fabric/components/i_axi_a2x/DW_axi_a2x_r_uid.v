...
//
//*************************************************************************************
module i_axi_a2x_DW_axi_a2x_r_uid (/*AUTOARG*/
   // Outputs
  ...
  localparam A2X_URID_PYLD_W             = (A2X_UPSIZE==1)? A2X_RUS_PYLD_W + 1 : (A2X_DOWNSIZE==1)? A2X_RDS_PYLD_W +1 : `i_axi_a2x_A2X_BSW+1;
  ...
  input                                       arvalid_sp;
  input  [`i_axi_a2x_A2X_IDW-1:0]                       arid_sp;
  ...
  // Signal Decelaration
  // **************************************************************************************
  reg    [`i_axi_a2x_A2X_IDW-1:0]                       uid_r;                 // Unique ID Value
  reg                                         uid_active_r;
  wire                                        rid_match;
  wire                                        aid_match;
  ...
  wire  [`i_axi_a2x_A2X_IDW-1:0]                        rid_sp;    
  wire  [A2X_SP_DW-1:0]                       rdata_sp; 
  wire  [`i_axi_a2x_A2X_RRESPW-1:0]                     rresp_sp; 
  ...
  wire   [`i_axi_a2x_A2X_BSW-1:0]                       r_pp_size;
  wire   [`i_axi_a2x_A2X_BSW-1:0]                       r_sp_ds_size;
  wire   [`i_axi_a2x_A2X_BSW-1:0]                       r_sp_us_size;
  wire   [`i_axi_a2x_A2X_BSW-1:0]                       r_sp_size;
  wire                                        r_last_int;
  ...
  if ((A2X_PP_MODE==1) && (A2X_PP_ENDIAN!=0)) begin: LE_TO_BE8     
    ...
    if (A2X_DOWNSIZE==1) begin
      assign r_pp_size = uid_pyld_o[(`i_axi_a2x_A2X_BSW+A2X_PP_NUM_BYTES_LOG2+2)-1:A2X_PP_NUM_BYTES_LOG2+2];
    end else if (A2X_UPSIZE==1) begin
      assign r_pp_size =  uid_pyld_o[(`i_axi_a2x_A2X_BSW+A2X_SP_NUM_BYTES_LOG2+2)-1:A2X_SP_NUM_BYTES_LOG2+2];
    end else begin
  ...
  generate 
    if (A2X_UPSIZE==1) begin: UPSIZE_UNPK
      i_axi_a2x_DW_axi_a2x_r_upk_dec
       #(
  ...
  if (A2X_SP_ENDIAN!=0) begin: BE8_TO_LE     
    wire   [`i_axi_a2x_A2X_BSW-1:0]                       r_pp_ds_size_w1;
    wire   [`i_axi_a2x_A2X_BSW-1:0]                       r_pp_us_size_w1;
    wire   [`i_axi_a2x_A2X_BSW-1:0]                       r_pp_size_w1;

    if (A2X_DOWNSIZE==1) begin
      assign r_pp_ds_size_w1 = uid_pyld_o[(`i_axi_a2x_A2X_BSW+A2X_PP_NUM_BYTES_LOG2+2)-1:A2X_PP_NUM_BYTES_LOG2+2];
      assign r_sp_size    = (r_pp_ds_size_w1<A2X_SP_MAX_SIZE) ? r_pp_ds_size_w1:A2X_SP_MAX_SIZE;
    end else if (A2X_UPSIZE==1) begin
      assign r_pp_us_size_w1 = uid_pyld_o[(`i_axi_a2x_A2X_BSW+A2X_SP_NUM_BYTES_LOG2+2)-1:A2X_SP_NUM_BYTES_LOG2+2];
    ...
    i_axi_a2x_DW_axi_a2x_x2x_et
     #(
      ...
      ,.A2X_UBB                (`i_axi_a2x_A2X_RUSER_BITS_PER_BYTE)      
