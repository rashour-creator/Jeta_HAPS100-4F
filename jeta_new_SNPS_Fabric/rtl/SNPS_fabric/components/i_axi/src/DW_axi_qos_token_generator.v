...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_qos_token_generator.v#7 $ 
--
...
module i_axi_DW_axi_qos_token_generator (
...
input    [`i_axi_REG_XCT_RATE_W-1:0]           reg_xct_rate_i                  ;
input    [`i_axi_REG_BURSTINESS_W-1:0]         reg_burstiness_i                ;
input                                    reg_slv_rdy_i                   ;
...
wire   [`i_axi_REG_XCT_RATE_W:0]       xct_rate_cnt               ;
...
wire                             token_inc                  ;
                 // token incrementer  
reg   [`i_axi_REG_XCT_RATE_W:0]        xct_rate_cnt_r             ;
                 // registered xct_rate_cnt  
reg   [`i_axi_REG_BURSTINESS_W:0]      token_cnt                  ;
                 // token counter 
...
//------------------------------------------------------------------------------
assign token_inc = xct_rate_cnt[`i_axi_REG_XCT_RATE_W];
...
begin: REG_PROC
   if (~aresetn_i)
   begin
      xct_rate_cnt_r <= {(`i_axi_REG_XCT_RATE_W+1){1'b0}};
...
assign xct_rate_cnt = reg_regulation_enable_i ? 
                         reg_slv_rdy_i ? 
                            (slv_rdy || (!valid_token_o)) ? 
                               (xct_rate_cnt_r[`i_axi_REG_XCT_RATE_W-1:0] + 
                                reg_xct_rate_i) : 
                                {1'b0,xct_rate_cnt_r[`i_axi_REG_XCT_RATE_W-1:0]} :
                            (xct_rate_cnt_r[`i_axi_REG_XCT_RATE_W-1:0] + 
                            reg_xct_rate_i) :
                         {(`i_axi_REG_XCT_RATE_W+1){1'b0}};
...
begin: token_cnt_PROC
   if (~aresetn_i)   
   begin
           token_cnt <= {(`i_axi_REG_BURSTINESS_W+1){1'b0}};
           valid_token_o <= 1'b0;
   end
   else if (!reg_regulation_enable_i || 
           (reg_xct_rate_i == {`i_axi_REG_XCT_RATE_W{1'b0}}) ||
            regulation_enabled_pulse)
   begin
           token_cnt <=  {{(`i_axi_REG_BURSTINESS_W){1'b0}},1'b1};
           valid_token_o <= 1'b1;
   end
   ...
   begin
           token_cnt <= (token_cnt - ( 1'b1));
           if (token_cnt == {{(`i_axi_REG_BURSTINESS_W){1'b0}},1'b1})
           valid_token_o <= 1'b0;
   end
...
assign next_token_o = ((token_cnt > {{(`i_axi_REG_BURSTINESS_W){1'b0}},1'b1}) || token_inc);
