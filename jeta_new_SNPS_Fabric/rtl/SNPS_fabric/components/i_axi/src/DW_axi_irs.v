...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_irs.v#12 $ 
**
** ---------------------------------------------------------------------
**
** File     : DW_axi_irs.v
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_W = `i_axi_AXI_AR_PYLD_M_W; // Width of payload signals in 
                                       // this block.    
...
//----------------------------------------------------------------------
  ...
  //SJ: This warning can be ignored. Its an temporary variable. 
  ...
  // payload source.
  //--------------------------------------------------------------------
  //ccx_cond: ; ; 0 ; masking the valid signal of read transfers from slave would cover this. 
  ...
  begin : frwd_mask_valid_r_PROC
    if(!aresetn_i) begin
      ...
      if(~frwd_mask_valid_r) begin
        ...
        //ccx_line: ; masking the valid signal of read transfers from slave would cover this.
 ...
 //spyglass enable_block FlopEConst
  ...
  always @(*) begin : full_mask_fdbk_mux_PROC
    ...
    if({full_mask_valid2_r,full_mask_valid1_r} == 2'b01) begin
      //ccx_line_begin: ; masking the valid signal of read transfers from slave would cover this.
  ...
  //--------------------------------------------------------------------
  assign bus_valid_o = (TMO==`i_axi_AXI_TMO_COMB)
                       ? bus_valid_i : 
                         ((TMO==`i_axi_AXI_TMO_FRWD) 
                          ? frwd_bus_valid_mskd
                          : full_valid_mux
                         ); 

  assign payload_o = (TMO==`i_axi_AXI_TMO_COMB)
                       ? payload_i : 
                         ((TMO==`i_axi_AXI_TMO_FRWD) 
                           ? frwd_payload_r 
                           : full_payload_mux
  ...
  assign ready_o = (TMO==`i_axi_AXI_TMO_COMB)
                     ? ready_i : 
                       ((TMO==`i_axi_AXI_TMO_FRWD) 
                         ? frwd_ready_o 
                         : full_ready
  ...
  assign id_o = (DO_MASKING & (TMO!=`i_axi_AXI_TMO_COMB))
                ? ((TMO==`i_axi_AXI_TMO_FRWD)
                    ? frwd_id
                    : full_id
                  )
  ...
  assign local_slv_o = (DO_MASKING & (TMO!=`i_axi_AXI_TMO_COMB))
                       ? ((TMO==`i_axi_AXI_TMO_FRWD)
                           ? frwd_local_slv
                           : full_local_slv
                         )
