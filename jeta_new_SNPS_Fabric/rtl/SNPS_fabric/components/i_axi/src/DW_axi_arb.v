...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_arb.v#15 $ 
**
** ---------------------------------------------------------------------
**
** File             : DW_axi_arb.v
...
//----------------------------------------------------------------------
  ...
  localparam MASTERS_PRIORITY_W =(ARB_TYPE==4)? `i_axi_AXI_QOSW: PRIORITY_W;

//----------------------------------------------------------------------
// LOCAL MACROS.
//----------------------------------------------------------------------
  localparam ARB_PARK_INDEX   = 0; // Macro for park index parameter of
                              // instantiated designware arbiter.
            
  localparam ARB_OP_MODE_NOREG = 0; // Macro for output mode parameter of
  ...
  localparam NC_INTERNAL = ((NC==1) ? 2 : NC);           

  // Macro for priority bus to the arbiter, using NC_INTERNAL macro.
  localparam BUS_PRIOR_W_INTERNAL = (NC_INTERNAL*MASTERS_PRIORITY_W);
...
// Outputs - Channel Destination.
 ...
 //VP:: To avoid lint error assigned unused bits to 0
  ...
  assign tx_issued_plarb =   grant_arb_o  & (~tx_not_accepted_r | (!MCA_EN)) 
  ...
  */
  ...
  generate 
    case (ARB_TYPE)
      ...
      `i_axi_AXI_ARB_TYPE_DP : begin : gen_arb_type_dp
        
      assign bus_priority_o_int = {NC_INTERNAL*ARB_INDEX_W{1'b0}};
        i_axi_DW_axi_arbiter_dp
      ...
      `i_axi_AXI_ARB_TYPE_FCFS : begin : gen_arb_type_fcfs
        i_axi_DW_axi_arbiter_fcfs
      ...
      `i_axi_AXI_ARB_TYPE_2T : begin : gen_arb_type_2t
        i_axi_DW_axi_arbiter_fae
 ...
 `i_axi_AXI_ARB_TYPE_QOS : begin : gen_arb_type_qos
  ...
  //ccx_cond: ; ; 1 ; Ready is always active for this channel.
