...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_sp.v#10 $ 
**
** ---------------------------------------------------------------------
**
** File     : DW_axi_sp.v
...
//----------------------------------------------------------------------
  ...
  localparam BUS_AR_PYLD_S_W = AR_NVM*`i_axi_AXI_AR_PYLD_S_W; 

  // Width of concatenated write address channel payload vector for all
  // visible master ports.
  localparam BUS_AW_PYLD_S_W = AW_NVM*`i_axi_AXI_AW_PYLD_S_W; 

  // Width of concatenated write data channel payload vector for all
  // visible master ports.
  localparam BUS_W_PYLD_S_W = W_NVM*`i_axi_AXI_W_PYLD_S_W;   

  // Width of concatenated master priorities for all visible master 
  // ports.
  localparam AR_BUS_PRIORITY_W = `i_axi_AXI_MST_PRIORITY_W*AR_NVM;
  localparam AW_BUS_PRIORITY_W = `i_axi_AXI_MST_PRIORITY_W*AW_NVM;
  localparam W_BUS_PRIORITY_W  = `i_axi_AXI_MST_PRIORITY_W*W_NVM;
  
  localparam RPAYLOAD_W = `i_axi_AXI_R_PYLD_M_W 
                          + (`i_axi_AXI_LOG2_NM*`i_axi_AXI_IS_ICM_M1);

  localparam BPAYLOAD_W = `i_axi_AXI_B_PYLD_M_W 
                          + (`i_axi_AXI_LOG2_NM*`i_axi_AXI_IS_ICM_M1);
...
//----------------------------------------------------------------------
  ...
  output                        arvalid_o;   // Valid to external 
                                             // slave.
  output [`i_axi_AXI_AR_PYLD_S_W-1:0] arpayload_o; // Payload to external 
                                             // slave.
  ...
  input [`i_axi_AXI_R_PYLD_S_W-1:0]  rpayload_i; // Payload from external 
                                           // slave.
  ...
  output [`i_axi_AXI_AW_PYLD_S_W-1:0] awpayload_o; // Payload to external 
                                             // slave.
  ...
  output [`i_axi_AXI_W_PYLD_S_W-1:0]  wpayload_o; // Payload vector to 
                                            // external slave.
  ...
  input [`i_axi_AXI_B_PYLD_S_W-1:0]  bpayload_i; // Payload from external 
                                           // slave.
  ...
  generate
    if(REMOVE_AR == 0) begin : gen_ar_addrch
      ...
      #(AR_NVM,             // Number of connected master ports.
        ...
        LOG2_NUM_VIS_MP,    // 
        `i_axi_AXI_AR_PL_ARB,     // Pipeline AR channel arbiter ?
        ...
        `i_axi_AXI_AR_PYLD_S_W,   // Width of payload vector to slave.
        ...
        `i_axi_AXI_NOT_AW_CH      // Not a write address channel.
       )
     ...
     assign arpayload_o= {`i_axi_AXI_AR_PYLD_S_W{1'b0}};
  ...
  //--------------------------------------------------------------------
  i_axi_DW_axi_sp_drespch
  
  #(NUM_VIS_MP,         // Number of visible master ports.
    LOG2_NUM_VIS_MP,    // Log 2 of number of visible master ports.
    `i_axi_AXI_R_PYLD_M_W,    // Payload width to master.
    `i_axi_AXI_R_PYLD_S_W,    // Payload width from slave.
  ...
  #(`i_axi_AXI_R_TMO,        // Channel timing option.
    NUM_VIS_MP,        // Number of visible master ports.
    `i_axi_AXI_R_PL_ARB,     // Is channel arbiter pipelined.
  ...
  generate
    if(REMOVE_AW == 0) begin : gen_aw_addrch
      ...
      #(AW_NVM,                // Number of connected master ports.
        ...
        NUM_VIS_MP,            // Number of visible master ports.
        LOG2_NUM_VIS_MP,       // 
        `i_axi_AXI_AW_PL_ARB,        // Pipeline AW channel arbiter.
        ...
        BUS_AW_PYLD_S_W,       // Width of bus containing payloads 
                               // from all visible master ports.
        `i_axi_AXI_AW_PYLD_S_W,      // Width of payload vector to slave.
        ...
        AW_BUS_PRIORITY_W,     // Width of bus containing priorities of 
                               // all visible masters.
        LOCKING,               // Implement locking or not.      
        `i_axi_AXI_AW_CH,            // Write address channel.
  ...
  //SJ: This warning can be ignored.
      ...
      assign awpayload_o         = {`i_axi_AXI_AW_PYLD_S_W{1'b0}};
    ...
    if(REMOVE_W == 0) begin : gen_w_datach
      ...
      #(W_NVM,                 // Number of connected master ports and 
        ...
        AW_NVM,                // Num masters visible to dedicated AW
                               // channel.
        `i_axi_AXI_W_PL_ARB,         // Pipeline W channel arbiter.
        ...
        BUS_W_PYLD_S_W,        // Width of bus containing payloads 
                               // from all visible master ports.
        `i_axi_AXI_W_PYLD_S_W,       // Payload width to slave. 
        MAX_FAWC,              // Max. number of active write commands.
        LOG2_MAX_FAWC,       
      ...
      assign wpayload_o           = {`i_axi_AXI_W_PYLD_S_W{1'b0}};
  ...
  #(NUM_VIS_MP,         // Number of visible master ports.
    LOG2_NUM_VIS_MP,    // Log 2 of number of visible master ports.
    `i_axi_AXI_B_PYLD_M_W,    // Payload width to master.
    `i_axi_AXI_B_PYLD_S_W,    // Payload width to slave.
    ...
    `i_axi_AXI_NOT_R_CH,      // R or B channel ?
  ...
  #(`i_axi_AXI_B_TMO,        // Channel timing option.
    NUM_VIS_MP,        // Number of visible master ports.
    `i_axi_AXI_B_PL_ARB,     // Is channel arbiter pipelined.
