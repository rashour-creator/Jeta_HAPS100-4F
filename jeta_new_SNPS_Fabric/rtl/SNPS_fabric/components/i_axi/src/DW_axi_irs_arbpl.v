...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_irs_arbpl.v#12 $ 
**
** ---------------------------------------------------------------------
**
...
//----------------------------------------------------------------------
  ...
  wire [AW_NSM-1:0] issued_wtx_shrd_mst_oh;
  ...
  // payload source.
  //--------------------------------------------------------------------
  //ccx_cond: ; ; 0 ; Ready is always active for this channel.
  ...
  //--------------------------------------------------------------------
  //ccx_cond: ; ; 1 ; This condition in covered in some of other instances.
 ...
 //spyglass enable_block FlopEConst
  ...
  generate
    if(AW_SHRD_DDCTD_LNK) begin
      assign   issued_wtx_shrd_mst_oh = frwd_ready_o ? issued_wtx_shrd_mst_oh_i : issued_wtx_shrd_mst_oh_r;
    end else begin
      assign   issued_wtx_shrd_mst_oh = {AW_NSM{1'b0}};
    end
  endgenerate
  ...
  assign payload_prereg_o = (TMO==`i_axi_AXI_TMO_COMB)
                            ? payload_i
