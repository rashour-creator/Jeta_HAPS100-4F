...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_sp_addrch.v#17 $ 
**
** ---------------------------------------------------------------------
**
...
//----------------------------------------------------------------------
  ...
  parameter BUS_PYLD_S_W = (2*`i_axi_AXI_AR_PYLD_S_W);
  
  // Width of payload vector to slave.
  parameter PYLD_S_W = `i_axi_AXI_AR_PYLD_S_W; 
...
// Width of Bus Priority Signals In case QOS if selected                           
 localparam  MASTER_BUS_PRIORITY_W = (ARB_TYPE==4) ? (`i_axi_AXI_QOSW * NUM_CON_MP) : BUS_PRIORITY_W;
...
//----------------------------------------------------------------------
 ...
 localparam REG_INTER_BLOCK_PATHS = 0;
...
//----------------------------------------------------------------------
  ...
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] s_on_w_shrd_only_bus_max;
  // Max sized bus of all AXI_S*_ON_AW_SHARED_VAL parameters.
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] s_on_aw_shrd_bus_max;
  ...
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] max_fac_s_bus_max;
  ...
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] tx_acc_s_bus_max;

  // Registered copy of cpl_tx_shrd_bus_i.
  reg [NSS-1:0] cpl_tx_shrd_bus_r; 
  
  // Max sized version of cpl_tx_shrd_bus_r.
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] cpl_tx_r_bus_max;

  // Max sized bus of tx_cnt_nz_s* signals.
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] tx_cnt_nz_s_bus_max;
  ...
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] has_ddctd_s_bus_max;
  // Above sized for NSS.
  ...
  assign bus_arvalid_shrd_masked 
    = SHARED ? bus_arvalid_i & (~arvalid_shrd_mask | ((NUM_CON_MP == 1) & (`i_axi_AXI_MAX_FAC_EN == 0)))
             : bus_arvalid_i;

  assign bus_awvalid_shrd_masked 
    = SHARED ? bus_awvalid_i & (~awvalid_shrd_mask | ((NUM_CON_MP == 1) & (`i_axi_AXI_MAX_FAC_EN == 0)))
             : bus_awvalid_i;
...
//tool issue is fixed
  ...
  generate
   if (`i_axi_AXI_MAX_NUM_MST_SLVS == NSS)
    assign tx_acc_s_bus_max = tx_acc_s_bus;
   else 
    assign tx_acc_s_bus_max = {{(`i_axi_AXI_MAX_NUM_MST_SLVS - NSS){1'b0}},tx_acc_s_bus};
  endgenerate  
  ...
  generate
   if (`i_axi_AXI_MAX_NUM_MST_SLVS == NSS)
    ...
    assign cpl_tx_r_bus_max = {{(`i_axi_AXI_MAX_NUM_MST_SLVS - NSS){1'b0}},cpl_tx_shrd_bus_r};
  endgenerate 
  ...
  assign s_on_w_shrd_only_bus_max = 
    {(`i_axi_AXI_S16_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S15_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S14_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S13_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S12_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S11_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S10_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S9_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S8_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S7_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S6_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S5_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S4_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S3_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S2_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S1_ON_W_SHARED_ONLY_VAL == 1),
     (`i_axi_AXI_S0_ON_W_SHARED_ONLY_VAL == 1)
    };

  assign s_on_aw_shrd_bus_max = 
    {(`i_axi_AXI_S16_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S15_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S14_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S13_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S12_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S11_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S10_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S9_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S8_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S7_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S6_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S5_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S4_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S3_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S2_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S1_ON_AW_SHARED_VAL == 1),
     (`i_axi_AXI_S0_ON_AW_SHARED_VAL == 1)
    };
  ...
  always @(*) begin : s_on_w_shrd_only_s_bus_PROC
    ...
    // can occur.
    for(slv=0;slv<`i_axi_AXI_MAX_NUM_MST_SLVS;slv=slv+1) begin
   ...
   begin
    assign max_fac_s_bus_masked = 
      ...
      & (~(  s_on_w_shrd_only_s_bus
          & {NSS{WCH & (`i_axi_AXI_W_SHARED_LAYER_NM == 1) & (`i_axi_AXI_MAX_FAC_EN == 0) & `i_axi_AXI_W_HAS_SHARED_LAYER}}
         ));
   end
   else
   begin
  ...
  // Bit for each slave on shared AW bus (applies to writes only),
  // asserted when a t/x for that slave has been issued.
  /*ccx_cond: ; 0 ; 1 ; rs_issued_tx_shrd is derived from the parameters, if the channel is shared then this signal is true.*/
  ...
  */
  /*ccx_cond: ; ; 1 ; rs_issued_tx_shrd is derived from the parameters, if the channel is shared then this signal is true. */ 
  ...
  #(((SHARED & SHARED_PL) ? `i_axi_AXI_TMO_FRWD : 0), // Channel timing option
   ...
   always @(*) begin : bus_valid_irs_arbpl_in_PROC
     ...
     for(slv=0;slv<NSS;slv=slv+1) begin
       ...
       if(has_ddctd_s_bus[slv] & SHARED_ARB_PL_IRS) begin
         //ccx_line: ; SHARED_ARB_PL_IRS is 0, this statement will not be covered in this configuration.
  ...
  //ccx_cond: ; ; 1 ; This condition is covered if atleast one of the slaves is accessed through dedicated address channel.
  assign payload_o 
