...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_aw_mtile_dlock.v#10 $ 
**
** ---------------------------------------------------------------------
**
...
module i_axi_DW_axi_aw_mtile_dlock (
  // Inputs 
  aclk_i,
  aresetn_i,
  bus_valid_i,
 ...
 */
 ...
 localparam WLAST_B4_AW = 
     (`i_axi_AXI_NUM_MASTERS == 1)
   | (VIS_S0 & (`i_axi_AXI_NMV_S0 == 1))
   | (VIS_S1 & (`i_axi_AXI_NMV_S1 == 1))
   | (VIS_S2 & (`i_axi_AXI_NMV_S2 == 1))
   | (VIS_S3 & (`i_axi_AXI_NMV_S3 == 1))
   | (VIS_S4 & (`i_axi_AXI_NMV_S4 == 1))
   | (VIS_S5 & (`i_axi_AXI_NMV_S5 == 1))
   | (VIS_S6 & (`i_axi_AXI_NMV_S6 == 1))
   | (VIS_S7 & (`i_axi_AXI_NMV_S7 == 1))
   | (VIS_S8 & (`i_axi_AXI_NMV_S8 == 1))
   | (VIS_S9 & (`i_axi_AXI_NMV_S9 == 1))
   | (VIS_S10 & (`i_axi_AXI_NMV_S10 == 1))
   | (VIS_S11 & (`i_axi_AXI_NMV_S11 == 1))
   | (VIS_S12 & (`i_axi_AXI_NMV_S12 == 1))
   | (VIS_S13 & (`i_axi_AXI_NMV_S13 == 1))
   | (VIS_S14 & (`i_axi_AXI_NMV_S14 == 1))
   | (VIS_S15 & (`i_axi_AXI_NMV_S15 == 1))
   | (VIS_S16 & (`i_axi_AXI_NMV_S16 == 1));
 ...
 */
  ...
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] vis_bus;
  integer slv;
  assign vis_bus = 
    {(VIS_S16 ? 1'b1 : 1'b0),
     (VIS_S15 ? 1'b1 : 1'b0),
   ...
   */
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] non_lcl_slv_bus;
  assign non_lcl_slv_bus = 
    {(`i_axi_AXI_ACC_NON_LCL_SLV_S16 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S15 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S14 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S13 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S12 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S11 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S10 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S9 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S8 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S7 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S6 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S5 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S4 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S3 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S2 ? 1'b1 : 1'b0),
     (`i_axi_AXI_ACC_NON_LCL_SLV_S1 ? 1'b1 : 1'b0),
  ...
  always @(*) begin : vis_non_lcl_slv_bus_PROC
    ...
    for(ss=0;ss<`i_axi_AXI_MAX_NUM_MST_SLVS;ss=ss+1) begin
      if(vis_bus[ss]) begin
   ...
   begin: nlcl_pend_cnt_wrap_r_PROC
    if(~aresetn_i) begin
      ...
      if(~nlcl_pend_cnt_wrap_r) begin
       ...
       //ccx_line: ; Condtion covered when AWVALID comes before WVALID.
  ...
  //ccx_cond : ; 0 ; 1 ; This condition can be covered when transaction at non local slave is active, and the current valid is not to the same non local slave.
