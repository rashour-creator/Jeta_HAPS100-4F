...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_qos_req_filter.v#7 $ 
--
...
module i_axi_DW_axi_qos_req_filter (
...
input    [`i_axi_REG_PEAK_RATE_W-1:0]          reg_peak_rate_i                 ;
...
//------------------------------------------------------------------------------
// Based on the slave visibility, the particular request is unconnected in the top level DW_axi file.
wire   [`i_axi_REG_PEAK_RATE_W:0]      peak_rate_cnt     ; // peak rate counter
wire                             peak_rate_flg_set ; // peak rate flag set
reg    [`i_axi_REG_PEAK_RATE_W:0]      peak_rate_cnt_r   ; // registered peak_rate_cnt
...
assign peak_rate_cnt = reg_regulation_enable_i ? 
                       (peak_rate_cnt_r[`i_axi_REG_PEAK_RATE_W-1:0] + 
                       reg_peak_rate_i) :{`i_axi_REG_PEAK_RATE_W{1'b0}};
// spyglass enable_block W164b

//------------------------------------------------------------------------------
// peak_rate_flg_set is asserted when peak_rate_cnt MSB is set to 1.
//------------------------------------------------------------------------------
assign peak_rate_flg_set = peak_rate_cnt[`i_axi_REG_PEAK_RATE_W];
...
begin: peak_rate_cnt_r_PROC
   if (~aresetn_i)   peak_rate_cnt_r <= {(`i_axi_REG_PEAK_RATE_W+1){1'b0}};
...
begin:peak_rate_flg_PROC 
   if (~aresetn_i)   peak_rate_flg <= 1'b0;
           ...
           (reg_peak_rate_i == {`i_axi_REG_PEAK_RATE_W{1'b0}}))) peak_rate_flg <= 1'b1;
...
//------------------------------------------------------------------------------
assign req_o = (next_req | reg_regulation_enable_i == 1'b0) ? req_i : {NC{1'b0}};
