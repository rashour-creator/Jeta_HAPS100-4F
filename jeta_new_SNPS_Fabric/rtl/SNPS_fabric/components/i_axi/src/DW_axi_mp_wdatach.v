...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_mp_wdatach.v#14 $ 
**
** ---------------------------------------------------------------------
**
...
module i_axi_DW_axi_mp_wdatach (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_M_W = `i_axi_AXI_W_PYLD_M_W; // W channel payload width
                                        // from master.
  parameter PYLD_S_W = `i_axi_AXI_W_PYLD_S_W; // W channel payload width to
                                        // slave.
...
//----------------------------------------------------------------------
  ...
  localparam S0_VIS  = ( S0_N_VIS ||  S0_B_VIS);
  localparam S1_VIS  = ( S1_N_VIS ||  S1_B_VIS);
  localparam S2_VIS  = ( S2_N_VIS ||  S2_B_VIS);
  localparam S3_VIS  = ( S3_N_VIS ||  S3_B_VIS);
  localparam S4_VIS  = ( S4_N_VIS ||  S4_B_VIS);
  localparam S5_VIS  = ( S5_N_VIS ||  S5_B_VIS);
  localparam S6_VIS  = ( S6_N_VIS ||  S6_B_VIS);
  localparam S7_VIS  = ( S7_N_VIS ||  S7_B_VIS);
  localparam S8_VIS  = ( S8_N_VIS ||  S8_B_VIS);
  localparam S9_VIS  = ( S9_N_VIS ||  S9_B_VIS);
  localparam S10_VIS = (S10_N_VIS || S10_B_VIS);
  localparam S11_VIS = (S11_N_VIS || S11_B_VIS);
  localparam S12_VIS = (S12_N_VIS || S12_B_VIS);
  localparam S13_VIS = (S13_N_VIS || S13_B_VIS);
  localparam S14_VIS = (S14_N_VIS || S14_B_VIS);
  localparam S15_VIS = (S15_N_VIS || S15_B_VIS);
  localparam S16_VIS = (S16_N_VIS || S16_B_VIS);

  localparam PAYLOAD_W = (PYLD_M_W + ICM_PORT*(`i_axi_AXI_LOG2_NM));
  localparam ID_W = (`i_axi_AXI_MIDW + ICM_PORT*(`i_axi_AXI_LOG2_NM));
...
//----------------------------------------------------------------------
  ...
  wire [`i_axi_AXI_NSP1-1:0] sys_slv_oh; // One hot system slave number.
  // Max sized version of sys_slv_oh.
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] sys_slv_oh_max;

  wire valid_mskd; // Masked version of valid_i from the id mask
                   // block.

  wire [`i_axi_AXI_LOG2_NM-1:0] ms_num; // Wire to hold the master system
  ...
  // registered depending on `i_axi_AXI_REG_AW_W_PATHS.
  ...
  assign act_ids_mux = `i_axi_AXI_REG_AW_W_PATHS ? act_ids_r : act_ids_i;
  assign act_snums_mux = `i_axi_AXI_REG_AW_W_PATHS ? act_snums_r : act_snums_i;
   ...
   begin : payload_id_assign_PROC
    integer pyld_bit;
        for(pyld_bit=0 ; pyld_bit<=(PAYLOAD_W-1) ; pyld_bit=pyld_bit+1) begin
            if ( pyld_bit <`i_axi_AXI_WPYLD_ID_RHS_M+ID_W  && pyld_bit >= `i_axi_AXI_WPYLD_ID_RHS_M)
                payload_i_wire[pyld_bit] = id_dummy[pyld_bit-`i_axi_AXI_WPYLD_ID_RHS_M] ;
  ...
  assign last = payload_i_wire[`i_axi_AXI_WPYLD_LAST];
  ...
  // Are AW to W paths registered.
  assign issuedtx_slot_oh_mux = `i_axi_AXI_REG_AW_W_PATHS
                                ? issuedtx_slot_oh_r
                                : issuedtx_slot_oh_i;
  ...
  i_axi_DW_axi_lcltosys
  
  #(NUM_VIS_SP,       // Number of local slaves.
    LOG2_NUM_VIS_SP,
    `i_axi_AXI_NSP1,        // Number of slaves on this interconnect instance.
    `i_axi_AXI_LOG2_NSP1,
  ...
  assign sys_slv_oh_max = {
                           {(`i_axi_AXI_MAX_NUM_MST_SLVS -`i_axi_AXI_NSP1){1'b0}},
                           sys_slv_oh};
  ...
  generate
   if (`i_axi_AXI_NUM_SYS_MASTERS == 1 || ICM_PORT == 1)
   begin
    always @(*)
   ...
   begin : non_icm_port_PROC
    reg [`i_axi_AXI_SIDW-1:0] sidw;
    always @(*)
    begin : append_mstnum_to_id_PROC
      integer pyld_bit;
      integer id_bit;
      ...
      id_bit = 0;
      sidw = {ms_num,
              payload_i_wire[`i_axi_AXI_WPYLD_ID_LHS_M:`i_axi_AXI_WPYLD_ID_RHS_M]};
      for(pyld_bit=0 ; pyld_bit<=(PYLD_S_W-1) ; pyld_bit=pyld_bit+1) begin
        // Assign fields up to id field.
        if(pyld_bit<`i_axi_AXI_WPYLD_ID_RHS_M) begin
          payload_o[pyld_bit] = payload_i_wire[pyld_bit];

        // Assign id field.
        end else if(   (pyld_bit>=`i_axi_AXI_WPYLD_ID_RHS_M)
                     && (pyld_bit<=`i_axi_AXI_WPYLD_ID_LHS_M+`i_axi_AXI_LOG2_NM)
                     )
        begin
        ...
        //ccx_line: ; This conditon not covered for channels without any sideband signals.
          payload_o[pyld_bit] = payload_i_wire[pyld_bit-`i_axi_AXI_LOG2_NM];
        end
        // spyglass enable_block SelfDeterminedExpr-ML      
      end
    end // append_mstnum_to_id_PROC
  ...
  //spyglass disable_block W164a W362 STARC05-2.10.3.1
  //SMD: Operator widths should be same in arithmetic operation 
  //SJ: RHS is a parameter which has default width of 32
  always @(posedge aclk_i or negedge aresetn_i)
  ...
  //spyglass enable_block W164a W362 STARC05-2.10.3.1
