...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_sp_wdatach.v#16 $ 
**
** ---------------------------------------------------------------------
**
...
module i_axi_DW_axi_sp_wdatach (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter BUS_PYLD_M_W = (2*`i_axi_AXI_W_PYLD_M_W); 

  parameter PYLD_S_W = `i_axi_AXI_W_PYLD_M_W; // Payload width to slave. 
...
//----------------------------------------------------------------------
  ...
  localparam ARB_NC = ((NUM_CON_MP==1) ? 1 : (SHARED ? NUM_CON_MP : WID));

  // Log base 2 of number of clients to the arbiter.
  localparam ARB_LOG2_NC = ((NUM_CON_MP==1) ? 1 : (SHARED ? LOG2_NUM_CON_MP : LOG2_WID));
  
  // Log base 2 of (number of clients to the arbiter + 1).
  localparam ARB_LOG2_NC_P1 = ((NUM_CON_MP==1) ? 1 : (SHARED ? LOG2_NUM_CON_MP_P1 : LOG2_WID_P1));

  // Width of priorities bus from the wrorder block, priorities for
  // 0 to WID-1.
  localparam BUS_PRIORITY_WID_W = (WID*`i_axi_AXI_MST_PRIORITY_W);

  // Width of bus containing master numbers of masters which are
  // being allowed to access the arbiter.
  localparam BUS_PORT_REQ_W = (WID*LOG2_NUM_CON_MP);

  localparam USE_INT_GRANT_INDEX = 0;
...
//----------------------------------------------------------------------
  ...
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] aw_shrd_param_bus;
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] w_shrd_param_bus;
  ...
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] vis_bus;
  ...
  always @(*) begin : aw_shrd_param_bus_lcl_PROC
    ...
    for(sm=0;sm<`i_axi_AXI_MAX_NUM_USR_MSTS;sm=sm+1) begin
      if(vis_bus[sm]) begin
        //ccx_line_begin: ; Some of the Masters are not visible to slave in this configuration. In that case some of the statements in for loop will not be covered. 
  ...
  begin : issued_tx_mst_combined_PROC
    ...
    for(mnum=0 ; mnum<=(NUM_VIS_MP-1) ; mnum=mnum+1) begin
      //ccx_line: ; Some of the Masters are not visible to slave in this configuration. In that case some of the statements in for loop will not be covered. 
  ...
  */
  ...
  #(ARB_TYPE,                 // Arbitration type.
    ...
    `i_axi_AXI_MST_PRIORITY_W,      // Priority width of a single master.            
  ...
  // by the arbiter.
  //ccx_cond : ; 0 ; 1 ; This condition cannot be covered as the value of SHARED is hardcoded to 0.
  ...
  assign cpl_tx = payload_pre_irs[`i_axi_AXI_WPYLD_LAST] & valid_o & ready_i;

  // Shared Channel
  // Detect write data part completion for each slave attached to the
  // shared channel.
  assign cpl_tx_shrd_bus = 
    (  {NSS{payload_pre_irs[`i_axi_AXI_WPYLD_LAST]}}
  ...
  //ccx_cond: ; ; 1 ; This condition is covered if atleast one of the slaves is accessed through dedicated data channel.
  assign payload_o 
