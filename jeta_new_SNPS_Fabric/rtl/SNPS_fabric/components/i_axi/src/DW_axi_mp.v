...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_mp.v#14 $ 
**
** ---------------------------------------------------------------------
**
** File     : DW_axi_mp.v
...
module i_axi_DW_axi_mp (
  ...
  has_ar_pending_trans_o,
  has_aw_pending_trans_o,
...
//----------------------------------------------------------------------
  ...
  localparam ICM_PYLD = `i_axi_AXI_HAS_BICMD*`i_axi_AXI_LOG2_NM;
  ...
  localparam BUS_R_PYLD_S_W
             = R_NVS*(`i_axi_AXI_R_PYLD_M_W + ICM_PYLD);
  ...
  localparam BUS_B_PYLD_S_W
             = B_NVS*(`i_axi_AXI_B_PYLD_M_W + ICM_PYLD);

  // Width of concatenated slave priorities for all visible slave ports.
  localparam R_BUS_PRIORITY_W = `i_axi_AXI_SLV_PRIORITY_W*R_NVS;
  localparam B_BUS_PRIORITY_W = `i_axi_AXI_SLV_PRIORITY_W*B_NVS;

  // Width of active read IDs bus.
  localparam ACT_RIDS_W
             = MAX_URIDA_M*(`i_axi_AXI_MIDW + ICM_PORT*(`i_axi_AXI_LOG2_NM));
  ...
  localparam ACT_WIDS_W
             = MAX_UWIDA_M*(`i_axi_AXI_MIDW + ICM_PORT*(`i_axi_AXI_LOG2_NM));
  ...
  localparam AW_IRS_ARB_PL_DO_MASKING
    = (`i_axi_AXI_AW_TMO==0) & (`i_axi_AXI_AW_PL_ARB==1);

  localparam AR_IRS_DO_MASKING
    = (`i_axi_AXI_AR_TMO!=0);
  localparam AR_IRS_ARB_PL_DO_MASKING
    = (`i_axi_AXI_AR_TMO==0) & (`i_axi_AXI_AR_PL_ARB==1);

  localparam W_IRS_DO_MASKING
    = (`i_axi_AXI_W_TMO!=0);
  localparam W_IRS_ARB_PL_DO_MASKING
    = (`i_axi_AXI_W_TMO==0) & (`i_axi_AXI_W_PL_ARB==1);

  // Must use the slave ID width if this is an ICM port.
  localparam AW_PYLD_ID_RHS = ICM_PORT
                              ? `i_axi_AXI_AWPYLD_ID_RHS_S
                              : `i_axi_AXI_AWPYLD_ID_RHS_M;

  localparam AW_PYLD_ID_LHS = ICM_PORT
                              ? `i_axi_AXI_AWPYLD_ID_LHS_S
                              : `i_axi_AXI_AWPYLD_ID_LHS_M;

  localparam AR_PYLD_ID_RHS = ICM_PORT
                              ? `i_axi_AXI_ARPYLD_ID_RHS_S
                              : `i_axi_AXI_ARPYLD_ID_RHS_M;

  localparam AR_PYLD_ID_LHS = ICM_PORT
                              ? `i_axi_AXI_ARPYLD_ID_LHS_S
                              : `i_axi_AXI_ARPYLD_ID_LHS_M;

  localparam W_PYLD_ID_RHS = ICM_PORT
                             ? `i_axi_AXI_WPYLD_ID_RHS_S
                             : `i_axi_AXI_WPYLD_ID_RHS_M;

  localparam W_PYLD_ID_LHS = ICM_PORT
                             ? `i_axi_AXI_WPYLD_ID_LHS_S
                             : `i_axi_AXI_WPYLD_ID_LHS_M;
...
//----------------------------------------------------------------------
  ...
  localparam LCL_AR_PYLD_W = `i_axi_AXI_AR_PYLD_M_W;
  localparam LCL_AW_PYLD_W = `i_axi_AXI_AW_PYLD_M_W;
  localparam LCL_W_PYLD_W  = `i_axi_AXI_W_PYLD_M_W;

  localparam LCL_R_M_PYLD_W = `i_axi_AXI_R_PYLD_M_W;
  localparam LCL_R_S_PYLD_W = `i_axi_AXI_R_PYLD_M_W;

  localparam LCL_B_M_PYLD_W = `i_axi_AXI_B_PYLD_M_W;
  localparam LCL_B_S_PYLD_W = `i_axi_AXI_B_PYLD_M_W;

  localparam ID_W = `i_axi_AXI_MIDW;

  localparam R_PYLD_ICM_W = LCL_R_M_PYLD_W;
  localparam B_PYLD_ICM_W = LCL_B_M_PYLD_W;
...
//----------------------------------------------------------------------
  ...
  output                        ar_shrd_ch_req_o; // Request for AR
                                                  // shared layer.
  output [`i_axi_AXI_AR_PYLD_S_W-1:0] arpayload_o;   // Payload to slave
                                               // ports.
  ...
  output                        aw_shrd_ch_req_o; // Request for AW
                                                  // shared layer.
  output [`i_axi_AXI_AW_PYLD_S_W-1:0] awpayload_o;   // Payload to master
                                               // interfaces.
  ...
  output                       w_shrd_ch_req_o; // Request for W
                                                // shared layer.
  output [`i_axi_AXI_W_PYLD_S_W-1:0] wpayload_o;   // Payload to slave ports.
  ...
  output [LCL_B_M_PYLD_W-1:0] bpayload_o; // Payload vector to external
                                           // master.
  output                    has_ar_pending_trans_o;
  output                    has_aw_pending_trans_o;
  ...
  wire                        arready_mp;
  wire [`i_axi_AXI_AR_PYLD_S_W-1:0] arpayload_mp;
  wire [`i_axi_AXI_AR_PYLD_S_W-1:0] arpayload_prereg;
  ...
  wire                        arready_irs;
  wire [`i_axi_AXI_AR_PYLD_S_W-1:0] arpayload_irs;

  wire [NUM_VIS_SP-1:0]       bus_awvalid_mp;
  wire                        awvalid_mp;
  wire                        awmask_valid_mp;
  wire                        awready_mp;
  wire [`i_axi_AXI_AW_PYLD_S_W-1:0] awpayload_mp;
  wire [`i_axi_AXI_AW_PYLD_S_W-1:0] awpayload_prereg;
  ...
  wire                        awready_irs;
  wire [`i_axi_AXI_AW_PYLD_S_W-1:0] awpayload_irs;

  wire [NUM_VIS_SP-1:0]       bus_wvalid_mp;
  wire                        wvalid_mp;
  wire                        wmask_valid_mp;
  wire                        wready_mp;
  wire [`i_axi_AXI_W_PYLD_S_W-1:0]  wpayload_mp;
  ...
  wire                        wready_irs;
  wire [`i_axi_AXI_W_PYLD_S_W-1:0]  wpayload_irs;
  ...
  wire [`i_axi_AXI_W_PYLD_S_W-1:0] wpayload_prereg_unconn;
  ...
  // region signals
  //wire [`i_axi_AXI_REGIONW-1:0] arregion_o;
  //wire [`i_axi_AXI_REGIONW-1:0] awregion_o;
  ...
  #(MS_NUM,                   // Master port system number.
    ...
    `i_axi_AXI_AR_TMO,              // AR channel timing option.
    `i_axi_AXI_AR_PL_ARB,           // Is channel arbiter pipelined.
    `i_axi_AXI_AR_PYLD_M_W,         // AR channel payload width from master.
    `i_axi_AXI_AR_PYLD_S_W,         // AR channel payload width to slave.
  ...
  U_AR_DW_axi_mp_addrch (
    ...
    .has_pending_trans_o         (has_ar_pending_trans_o),
...
// spyglass enable_block W287b
  ...
  // the channel.
  assign ar_id_irs_mux = (`i_axi_AXI_AR_TMO!=0)
                         ? ar_id_irs
                         : (`i_axi_AXI_AR_PL_ARB==1)
                           ? ar_id_irs_arbpl
                           : {ID_W{1'b0}};

  assign ar_local_slv_irs_mux = (`i_axi_AXI_AR_TMO!=0)
                                ? ar_local_slv_irs
                                : (`i_axi_AXI_AR_PL_ARB==1)
  ...
  #(`i_axi_AXI_AR_TMO,          // Channel timing option.
    NUM_VIS_SP,           // Number of visible slave ports.
    `i_axi_AXI_AR_PL_ARB,       // Is channel arbiter pipelined.
    `i_axi_AXI_AR_PYLD_S_W,     // Channel payload width.
  ...
  generate
    if(REMOVE_R == 0) begin : gen_r_drespch
      ...
      //----------------------------------------------------------------
      i_axi_DW_axi_mp_drespch
      
      #(ICM_PORT,
        R_NVS,               // Number of visible slave ports.
        R_NVS_LOG2,          //
        R_NVS_P1_LOG2,       //
        `i_axi_AXI_R_PL_ARB,       // Pipeline channel arbiter outputs.
        ...
        LOG2_MAX_URIDA_M,    // Log base 2 of MAX_URIDA_M.
        ACT_RSNUMS_W,        // Width of active slave numbers bus.
        `i_axi_AXI_R_CH,           // Block is being used in a read data
                             // channel here.
  ...
  #(MS_NUM,                   // Master port system number.
    ...
    NUM_VIS_SP,               // Number of visible slave ports.
    LOG2_NUM_VIS_SP,          // Log 2 of number of visible slave ports.
    `i_axi_AXI_AW_TMO,              // AW channel timing option.
    `i_axi_AXI_AW_PL_ARB,           // Is channel arbiter pipelined.
    `i_axi_AXI_AW_PYLD_M_W,         // AW channel payload width from master.
    `i_axi_AXI_AW_PYLD_S_W,         // AW channel payload width to slave.
  ...
  U_AW_DW_axi_mp_addrch (
    ...
    .has_pending_trans_o         (has_aw_pending_trans_o),
  ...
  // Select masking feedback signals from the first pipeline stage in
  // the channel.
  assign aw_id_irs_mux = (`i_axi_AXI_AW_TMO!=0)
                         ? aw_id_irs
                         : (`i_axi_AXI_AW_PL_ARB==1)
                           ? aw_id_irs_arbpl
                           : {ID_W{1'b0}};

  assign aw_local_slv_irs_mux = (`i_axi_AXI_AW_TMO!=0)
                                ? aw_local_slv_irs
                                : (`i_axi_AXI_AW_PL_ARB==1)
  ...
  #(`i_axi_AXI_AW_TMO,          // Channel timing option.
    NUM_VIS_SP,           // Number of visible slave ports.
    `i_axi_AXI_AW_PL_ARB,       // Is channel arbiter pipelined.
    `i_axi_AXI_AW_PYLD_S_W,     // Channel payload width.
    LOG2_NUM_VIS_SP,      // Log base of num. visible slave ports.
    ID_W,                // Master ID width.
   ...
   */
  i_axi_DW_axi_aw_mtile_dlock
  
  #( .NUM_VIS_SP(NUM_VIS_SP)
    ,.PYLD_W(`i_axi_AXI_W_PYLD_S_W)
    ,.AW_PL_ARB(`i_axi_AXI_AW_PL_ARB)
    ,.W_PL_ARB(`i_axi_AXI_W_PL_ARB)
  ...
  U_DW_axi_aw_mtile_dlock (
    ...
    .wlast_i        (wpayload_o[`i_axi_AXI_WPYLD_LAST]),
    // Outputs
  ...
  //--------------------------------------------------------------------
  i_axi_DW_axi_mp_wdatach
  
  #(MS_NUM,                  // Master port system number.
    ICM_PORT,                // Interconnecting master port.
    NUM_VIS_SP,              // Number of visible slave ports.
    LOG2_NUM_VIS_SP,         // Log 2 of number of visible slave ports.
    `i_axi_AXI_W_TMO,              // W channel timing option.
    `i_axi_AXI_W_PL_ARB,           // Is channel arbiter pipelined.
    `i_axi_AXI_W_PYLD_M_W,         // W channel payload width from master.
    `i_axi_AXI_W_PYLD_S_W,         // W channel payload width to slave.
    ACT_WIDS_W,              // Width of active write IDs bus.
    ACT_WSNUMS_W,            // Width of active slave numbers bus.
    ACT_WCOUNT_W,            // Width of active write count per ID bus.
  ...
  // Select masking feedback signals from the first pipeline stage in
  // the channel.
  assign w_id_irs_mux = (`i_axi_AXI_W_TMO!=0)
                        ? w_id_irs
                        : (`i_axi_AXI_W_PL_ARB==1)
                          ? w_id_irs_arbpl
                          : {ID_W{1'b0}};
  ...
  #(`i_axi_AXI_W_TMO,          // Channel timing option.
    NUM_VIS_SP,          // Number of visible slave ports.
    `i_axi_AXI_W_PL_ARB,       // Is channel arbiter pipelined.
    `i_axi_AXI_W_PYLD_S_W,     // Channel payload width.
    LOG2_NUM_VIS_SP,     // Log base of num. visible slave ports.
    ID_W,               // Master ID width.
  ...
  generate
    if(REMOVE_B == 0) begin : gen_b_drespch
      ...
      #(ICM_PORT,            // Interconnecting master port.
        ...
        B_NVS_LOG2,          //
        B_NVS_P1_LOG2,       //
        `i_axi_AXI_B_PL_ARB,       // Pipeline channel arbiter outputs.
        ...
        LOG2_MAX_UWIDA_M,    // Log base 2 of MAX_UWIDA_M.
        ACT_WSNUMS_W,        // Width of active slave numbers bus.
        `i_axi_AXI_NOT_R_CH        // Block is not being used in a read data
                             // channel here.
       )
