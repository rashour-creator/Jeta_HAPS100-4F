...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_mp_idmask.v#15 $ 
**
** ---------------------------------------------------------------------
**
...
//----------------------------------------------------------------------
 ...
 // output [LOG2_MAX_UIDA*MAX_UIDA*MAX_CA_ID-1:0] act_ids_buffer; // buffer for active ids on address channel
  ...
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] shrd_here_has_shrd_ddctd_bus;
  ...
  // link.
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] shrd_ddctd_s_bus;
  ...
  always @(*) begin : shrd_here_has_shrd_ddctd_bus_vis_PROC
    ...
    for(ss=0;ss<`i_axi_AXI_MAX_NUM_USR_MSTS;ss=ss+1) begin
      if(vis_bus[ss]) begin
  ...
  begin : tx_in_shrd_ddctd_pipe_mask_r_PROC
    if(~aresetn_i) begin
      ...
      if(  local_slv_mux_thru_shrd_ddctd
         ...
         & (`i_axi_AXI_AW_PL_ARB | `i_axi_AXI_AW_SHARED_PL)
        ) begin
...
assign tx_issued_valid = valid_i & ready_mst_i ; // when a tx is accepted
  ...
  reg [ACT_ID_BUF_POINTER_WIDTH-1:0] act_ids_buffer_w;
  always @(posedge aclk_i or negedge aresetn_i)
  begin : act_ids_buffer_reg_PROC
    if(!aresetn_i) 
      act_ids_buffer_r <= {ACT_ID_BUF_POINTER_WIDTH{1'b0}};
    else 
      act_ids_buffer_r<= act_ids_buffer_w;
  end
  ...
  begin: act_ids_buffer_PROC 
    integer id_bit ;
    act_ids_buffer_w = act_ids_buffer_r;
    if(tx_issued_valid || (act_ids_rd_buffer_pointer == act_ids_buffer_pointer)) begin
       for(id_bit=0 ; id_bit<=(ID_W-1) ; id_bit=id_bit+1) begin
         act_ids_buffer_w[(act_ids_buffer_pointer*ID_W)+id_bit]
          = id_i[id_bit]; 
      end
    end   
  ...
  begin
    if(!aresetn_i) 
      ...
      no_act_id_r <= (act_ids_rd_buffer_pointer == act_ids_buffer_pointer);
  end

  generate 
    if (`i_axi_AXI_REG_AW_W_PATHS) begin
      assign no_act_id = (act_ids_rd_buffer_pointer == act_ids_buffer_pointer); 
      assign act_ids_buffer = act_ids_buffer_r; 
    end else begin
      assign no_act_id = (act_ids_rd_buffer_pointer == act_ids_buffer_pointer) & (~tx_issued_valid);
      assign act_ids_buffer = act_ids_buffer_w;
    end  
  endgenerate
