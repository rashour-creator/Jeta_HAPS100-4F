...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_arbpl.v#10 $ 
**
** ---------------------------------------------------------------------
**
** File     : DW_axi_arbpl.v
...
// Outputs - Multi cycle arbitration control.
  ...
  begin : mca_count_r_PROC
    if(~aresetn_i) begin
      ...
      if(mca_count_r == (MCA_NC - 1)) begin
        ...
        //ccx_line: ; Multicycle arbitration not enabled in this configuration. 
  ...
  begin : arb_outputs_reg_PROC
    if(~aresetn_i) begin
     ...
     if(apl_gi_update) begin
       // depending on Pipelining options and MCA_EN this signal may not be used
        grant_r       <= grant_i;
        bus_grant_r   <= bus_grant_i;
        grant_index_r <= grant_index_i;
      end else if (MCA_EN==1) begin 
        grant_r       <= (grant_index_update & take_mca_grant_i & grant_i);
        if (grant_index_update & take_mca_grant_i & grant_i) begin 
          bus_grant_r   <= bus_grant_i;
          grant_index_r <= grant_index_i;
        end 
      end
