...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_arbiter_fae.v#13 $ 
//
...
`include "DW_axi_all_includes.vh"
  ...
  module i_axi_DW_axi_arbiter_fae (
  clk,
  rst_n,
  init_n,
  enable,
...
// spyglass enable_block SelfDeterminedExpr-ML
  ...
  begin: S_ARB_STATE_MACHINE_PROC
    ...
    // PARKED
    2'b01: begin
     //ccx_line_begin: ; Arbiter is enabled only when request is asserted. The state moves from 0 to 2 and remains there until reset. 
      case (st_vec)
      2'b00: next_state = 2'b10;
    ...
    2'b10: begin
      case (st_vec)
      //ccx_line: ; Arbiter is enabled only when request is asserted. The state moves from 0 to 2 and remains there until reset. 
      2'b01: next_state = 2'b11;
      2'b10: next_state = 2'b01;
  ...
  begin:C_STATE_TRANSITION_PROC
    for (i3=0 ; i3<N ; i3=i3+1) begin
      for (l3=0 ; l3<INDEX_WIDTH ; l3=l3+1) begin
        ...
        2'b01: begin
         ...
         //ccx_line_end
            ...
            //ccx_line_end
                ...
                //ccx_line: ; Arbiter is enabled only when request is asserted. STATE 2'b01 and 2'b11 is never reached.
                  next_prior[i3*INDEX_WIDTH+l3] = decr_prior[i3*INDEX_WIDTH+l3];
                end
              end
            end
        ...
        default: begin
          if (next_locked) begin
          //ccx_line_begin: ; Lock signal is connected to 0. Hence this condition/statement is uncovered.
            if (masked_req[i3] == 1'b0) begin
              next_prior[i3*INDEX_WIDTH+l3] = max_prior[l3];
          //ccx_line_end
            end
            else begin
           //ccx_line: ; Lock signal is connected to 0. Hence this condition/statement is uncovered.
              next_prior[i3*INDEX_WIDTH+l3] = int_priority[i3*INDEX_WIDTH+l3];
            end
          end
          else begin
...
// spyglass enable_block SelfDeterminedExpr-ML      
  ...
  always @(posedge clk or negedge rst_n) begin : arb_reg_PROC
    if (~rst_n) begin
     ...
     //ccx_line_begin: ; init_n signal is driven to 1, this statement will never be covered.
      current_state       <= 2'b00;
      grant_index_int     <= {INDEX_WIDTH{1'b1}};
      parked_int          <= 1'b0;
      granted_int         <= 1'b0;
  ...
  begin
    always @(posedge clk or negedge rst_n) begin : INT_PRIORITY_PROC
      if (~rst_n)
       //ccx_line: ; init_n signal is driven to 1, this statement will never be covered.
        int_priority        <= {N*INDEX_WIDTH{1'b1}};
      else if (init_n == 1'b0) 
        int_priority        <= {N*INDEX_WIDTH{1'b1}};
      else if (enable) 
