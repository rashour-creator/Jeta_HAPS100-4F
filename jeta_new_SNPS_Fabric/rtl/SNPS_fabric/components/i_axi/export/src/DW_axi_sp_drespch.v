...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_sp_drespch.v#10 $ 
**
** ---------------------------------------------------------------------
**
...
module i_axi_DW_axi_sp_drespch (
  // Inputs - System.
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_M_W = `i_axi_AXI_R_PYLD_M_W; // Payload width to master.
  parameter PYLD_S_W = `i_axi_AXI_R_PYLD_S_W; // Payload width from slave.
...
// LOCAL MACROS.
//----------------------------------------------------------------------
  localparam DRESPCH_ID_RHS = ((R_CH==1) ? `i_axi_AXI_RPYLD_ID_RHS_S : `i_axi_AXI_BPYLD_ID_RHS_S);
  localparam DRESPCH_ID_LHS = ((R_CH==1) ? `i_axi_AXI_RPYLD_ID_LHS_S : `i_axi_AXI_BPYLD_ID_LHS_S);
...
//----------------------------------------------------------------------
  ...
  // REGISTER VARIABLES.
  //--------------------------------------------------------------------
  reg [`i_axi_AXI_LOG2_NM-1:0] id_mstnum; // Master number taken from 
                                    // slaves ID signal.
  ...
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] shared_m_bus;
  wire [`i_axi_AXI_SIDW-1:0] id_slv; // ID signal from slave.

  // One hot system master number from systolcl block.
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] bidi_sys_pnum_oh;
  reg [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] id_mstnum_oh;
  ...
  begin : C_ID_MSTNUM_PROC
   ...
   // 1 master in the system.
     for(mnum_bit=0 ; 
         mnum_bit<=(`i_axi_AXI_LOG2_NM-1) ; 
         mnum_bit=mnum_bit+1
        ) 
     begin
       id_mstnum[mnum_bit] = id_slv[mnum_bit+`i_axi_AXI_MIDW];
     end
  end // id_mstnum_PRO
  ...
  // slave port.
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] vis_m_bus;
  assign vis_m_bus
    = {(M15_VIS ? 1'b1 : 1'b0),
       (M14_VIS ? 1'b1 : 1'b0),
       (M13_VIS ? 1'b1 : 1'b0),
  ...
  always @(*) begin : id_mstnum_oh_PROC
    integer mst;
    id_mstnum_oh = {`i_axi_AXI_MAX_NUM_USR_MSTS{1'b0}};
    for(mst=0 ; mst<`i_axi_AXI_MAX_NUM_USR_MSTS ; mst=mst+1) begin
  ...
  assign mst_on_shrd = HAS_SHARED 
                       ? |(  shared_m_bus 
                           & (`i_axi_AXI_HAS_BICMD ? bidi_sys_pnum_oh 
                                             : id_mstnum_oh
                             )
                          )
                         : 1'b0 ;
  ...
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] shared_m_a_bus;
  ...
  #(
    ...
    `i_axi_AXI_NUM_MASTERS,  // Number of slaves in system, including default
                       // slave.
    `i_axi_AXI_LOG2_NM,      // Log base 2 of number of slaves in the system.
  ...
  // spyglass enable_block W528
    ...
    for(pyld_bit=0 ; pyld_bit<=(PYLD_M_W-1) ; pyld_bit=pyld_bit+1) begin
      ...
      // Assign id field.
                  ...
                  && (pyld_bit<=(DRESPCH_ID_LHS-`i_axi_AXI_LOG2_NM))
                 ) 
      begin
      ...
      // Assign fields after id fields.
        ...
        // SMD: Self determined expression found
        // SJ : The expression indexing the vector/array will never exceed the bound of the vector/array.        
          payload_o[pyld_bit] = payload_i[pyld_bit+`i_axi_AXI_LOG2_NM];
        // spyglass enable_block SelfDeterminedExpr-ML   
      end
    end
  ...
  // read data channel.
  //ccx_cond: ; ; 10 ; This condition gets covered when the same module is used for read data channel.
  assign cpl_tx_o = valid_i & ready_i && 
                    (payload_i[`i_axi_AXI_RPYLD_LAST] || (!R_CH));
