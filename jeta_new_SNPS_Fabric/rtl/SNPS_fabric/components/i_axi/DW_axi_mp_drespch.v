...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_mp_drespch.v#13 $ 
**
** ---------------------------------------------------------------------
**
...
module i_axi_DW_axi_mp_drespch (
  // Inputs - System.  
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  parameter BUS_PYLD_S_W = (4*`i_axi_AXI_R_PYLD_M_W);  // Width of bus with 
                                                 // payloads from all 
                                                 // visible slaves.

  parameter PYLD_M_W = `i_axi_AXI_R_PYLD_M_W;  // Width of payload bus to 
                                         // master.

  parameter PYLD_S_W = `i_axi_AXI_R_PYLD_M_W;  // Width of individual
  ...
  localparam SBW = (R_CH==1) ? `i_axi_AXI_R_SBW : `i_axi_AXI_B_SBW;
...
//----------------------------------------------------------------------
  ...
  localparam PER_SLAVE_ARBITRATION = 1;

  // Macros for arbiter parameters depending on whether we are 
  // arbitrating for all visible slaves or for the number of unique 
  // ID values.
  localparam ARB_NC = (PER_SLAVE_ARBITRATION ? NUM_VIS_SP : MAX_UIDA);
  localparam ARB_PRIOR_W = (PER_SLAVE_ARBITRATION ? BUS_PRIORITY_W : (MAX_UIDA*`i_axi_AXI_LOG2_NSP1));

  localparam ARB_LOG2_NC = (PER_SLAVE_ARBITRATION ? LOG2_NUM_VIS_SP : LOG2_MAX_UIDA);

  // Note this value only works for per slave arbitration.
  // Since we never use per UIDA arbitration at the moment.
  localparam ARB_LOG2_NC_P1 = LOG2_NUM_VIS_SP_P1;

  localparam BUS_PORT_REQ_W = (PER_SLAVE_ARBITRATION ? (NUM_VIS_SP*LOG2_NUM_VIS_SP) : ACT_SNUMS_W);

  // Switch ID field macros depending on which channel is being
  // implemented.
  localparam DRESPCH_ID_RHS = ((R_CH==1) ? `i_axi_AXI_RPYLD_ID_RHS_M : `i_axi_AXI_BPYLD_ID_RHS_M);
  localparam DRESPCH_ID_LHS = ((R_CH==1) ? `i_axi_AXI_RPYLD_ID_LHS_M : `i_axi_AXI_BPYLD_ID_LHS_M);

  localparam ID_W = (`i_axi_AXI_MIDW + ICM_PORT*(`i_axi_AXI_LOG2_NM));

  // Width of payload for ICM ports, only used if SHARED==1.
  localparam PYLD_ICM_W = (PYLD_M_W + ICM_PORT*(`i_axi_AXI_LOG2_NM));
...
//----------------------------------------------------------------------
  ...
  wire [`i_axi_AXI_MAX_NUM_MST_SLVS-2:0] has_ddctd_m_bus_max;
  // Above sized for NSM.
  ...
  DW_axi_multibusmux
  #(NUM_VIS_SP,           // Number of inputs to the mux.
    `i_axi_AXI_SLV_PRIORITY_W,  // Width of each input to the mux.
    LOG2_NUM_VIS_SP,      // Width of select line for the mux.
    MAX_UIDA              // Number of busmuxes to implement.
  )
  ...
  */
  ...
  #(ARB_TYPE,                 // Type of arbitration used.
    ...
    `i_axi_AXI_MCA_HLD_PRIOR,       // Hold priorities for multi cycle arb.
    `i_axi_AXI_SLV_PRIORITY_W,      // Priority width of a single slave.                 
  ...
  U_DW_axi_arb (
    ...
    .last_i               (payload_pre_irs[`i_axi_AXI_RPYLD_LAST]),
  ...
  assign cpl_tx_o 
   = R_CH 
     ? payload_o[`i_axi_AXI_RPYLD_LAST] & valid_o & ready_i
     : valid_o & ready_i;
  ...
  assign cpl_tx_shrd_bus_o 
    = {NSM{ ((payload_o[`i_axi_AXI_RPYLD_LAST] | (R_CH==0)) & ready_shrd_mux)  }}
      & bus_valid_shrd_o;                    
  ...
  //ccx_cond: ; ; 1 ; This condition is covered if atleast one of the master is accessed through dedicated response channel.
