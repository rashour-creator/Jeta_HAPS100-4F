...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_mp_addrch.v#11 $ 
**
** ---------------------------------------------------------------------
**
...
//----------------------------------------------------------------------
  ...
  parameter PYLD_M_W = `i_axi_AXI_AR_PYLD_M_W;
...
//----------------------------------------------------------------------
  ...
  //output [`i_axi_AXI_REGIONW-1:0]   region_o;

  output                    has_pending_trans_o;
  ...
  reg [`i_axi_AXI_MAX_NUM_MST_SLVS-1:0] xdcd_slvnum_oh;
  ...
  wire [`i_axi_AXI_AW-1:0]   addr_mst;

  // Wires for unconnected sub module ports.
  wire [`i_axi_AXI_LOG2_NSP1-1:0] sys_slv_unconn;
  wire [`i_axi_AXI_MAX_NUM_USR_MSTS-1:0] bidi_sys_pnum_oh_unconn;
  ...
  wire valid_lp;
  wire [`i_axi_AXI_REGIONW-1:0]   region_o;
 ...
 // wire [LOG2_MAX_UIDA*MAX_UIDA*MAX_CA_ID-1:0] act_ids_buffer; // buffer for active ids on address channel
  ...
  assign addr_mst
    = payload_i[`i_axi_AXI_ARPYLD_ADDR_LHS:`i_axi_AXI_ARPYLD_ADDR_RHS];

  assign id_mst
    = payload_i[`i_axi_AXI_AWPYLD_ID_RHS_M+ID_W-1:`i_axi_AXI_AWPYLD_ID_RHS_M];
  ...
  assign local_slv_mux = `i_axi_AXI_HAS_XDCDR
                         ? local_slv_xdcdr
                            : local_slv_idcdr;
 ...
 //   (`i_axi_AXI_HAS_TZ_SUPPORT & (local_slv_mux != local_slv_tzone))
 //   ? (SHARED_S0 == 1'b1)
 //   : `i_axi_AXI_HAS_XDCDR
 //     ? slv_on_shrd_xdcdr
 //     : slv_on_shrd_idcdr;
  ...
  assign slv_on_shrd_mux = (`i_axi_AXI_HAS_TZ_SUPPORT
    )
    ?
    (SHARED_S0 == 1'b1)
    : `i_axi_AXI_HAS_XDCDR
  ...
  //assign local_slv = `i_axi_AXI_HAS_TZ_SUPPORT
  //                   ? local_slv_tzone
  //                     : local_slv_mux;
  ...
  U_DW_axi_mp_idmask (
    ...
    .has_pending_trans_o         (has_pending_trans_o),
  ...
  generate 
   ...
   begin: non_icm_port_PROC
    ...
    begin : append_mstnum_to_id_PROC
      ...
      sidw = {MS_NUM,
              payload_i_q[`i_axi_AXI_ARPYLD_ID_LHS_M:`i_axi_AXI_ARPYLD_ID_RHS_M]};
      ...
      for(pyld_bit=0 ;
       ...
       if(pyld_bit<`i_axi_AXI_ARPYLD_ID_RHS_M) begin
          payload_o_int[pyld_bit] = payload_i_q[pyld_bit];

        // Assign id field.
        end else if(   (pyld_bit>=`i_axi_AXI_ARPYLD_ID_RHS_M)
                     && (pyld_bit<=(`i_axi_AXI_ARPYLD_ID_LHS_M+`i_axi_AXI_LOG2_NM))
                     )
        begin
        ...
        // SJ : The expression indexing the vector/array will never exceed the bound of the vector/array. 
         //ccx_line: ; This conditon not covered for channels without any sideband signals.
            payload_o_int[pyld_bit] = payload_i_q[pyld_bit-`i_axi_AXI_LOG2_NM];
