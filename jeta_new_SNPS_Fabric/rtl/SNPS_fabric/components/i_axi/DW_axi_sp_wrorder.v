...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_sp_wrorder.v#11 $ 
**
** ---------------------------------------------------------------------
**
...
module i_axi_DW_axi_sp_wrorder (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  // interleaving slots.
  localparam BUS_SLOT_MST_W = (LOG2_NUM_VIS_MP*WID);

  // Number of input busses to each mst require mux.
  localparam MST_REQ_MUX_NUM_DIN = 2;

  // Width of master require bus mux data in bus.
  localparam MST_REQ_BMUX_DIN_W = (MST_REQ_MUX_NUM_DIN*LOG2_NUM_VIS_MP);

  // Width of master require multi bus mux data in bus.
  localparam MST_REQ_MBMUX_DIN_W = (MST_REQ_BMUX_DIN_W*WID);
...
//----------------------------------------------------------------------
  ...
  assign can_send_vld_frm_aw =
    (   issued_tx_i
      ...
      & ((WID==1) | (`i_axi_AXI_AW_PL_ARB==1))
      & ((HAS_W_SHRD_DDCTD_LNK==0) | (`i_axi_AXI_AW_SHARED_PL==1))
      & (`i_axi_AXI_REG_AW_W_PATHS==0)
      & (MCA_EN==0)
    );
...
//spyglass enable_block W528
  ...
  // result in very long logic paths.  `i_axi_AXI_REG_AW_W_PATHS macro is
  // used to disable this feature.
  //ccx_cond: ; 0 ; 1 ; This condition will be covered when the fifo becomes empty.
  ...
  #(NUM_VIS_MP,           // Number of inputs to the mux.
    `i_axi_AXI_MST_PRIORITY_W,  // Width of each input to the mux.
    LOG2_NUM_VIS_MP,      // Width of select line for the mux.
    WID                   // Number of busmuxes to implement.
  )
