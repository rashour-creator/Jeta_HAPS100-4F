...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/src/DW_axi_sp_lockarb.v#18 $ 
**
** ---------------------------------------------------------------------
**
...
module i_axi_DW_axi_sp_lockarb (
  // Inputs - System.
  aclk_i,
  aresetn_i,
...
//----------------------------------------------------------------------
  ...
  localparam IMPLEMENT_LOCKING = ((LOCKING==1) && (NC>1));
...
//----------------------------------------------------------------------
  ...
  //ccx_cond: ; ; 1 ; Can be covered by setting parameter AXI_HAS_LOCKING =1 and internal parameter LOCKING=1.
  ...
  //--------------------------------------------------------------------
  assign max_act_txs = (((NC>1) || (`i_axi_AXI_MAX_FAC_EN==1)) & (SHARED==0))
  ...
  begin : unlocking_tx_rcvd_r_PROC
    if(!aresetn_i) begin
      ...
      begin
        //ccx_line: ; Can be covered by setting parameter AXI_HAS_LOCKING =1 and internal parameter LOCKING=1.
  ...
  //ccx_cond: ; 0 ; 1 ; This condition can be covered when grant is masked.
