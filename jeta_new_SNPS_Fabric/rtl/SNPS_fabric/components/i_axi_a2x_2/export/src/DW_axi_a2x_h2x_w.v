...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/axi_dev_br/src/DW_axi_a2x_h2x_w.v#6 $ 
// --------------------------------------------------------------------
*/
...
module i_axi_a2x_2_DW_axi_a2x_h2x_w (/*AUTOARG*/
   // Outputs
   w_hready_resp, w_split_resp, w_retry_resp, w_error_resp, w_hsplit, awvalid, aw_pyld, wvalid, w_pyld, bready, busy, w_buf_full,
   // Inputs
  ...
  input                                    hmastlock; 
  input  [`i_axi_a2x_2_A2X_HBLW-1:0]                   hburst_dp;  
  //spyglass disable_block W240
  //SMD: An input has been declared but is not read.
  //SJ : These signals are used in specific config only 
  input  [2:0]                             hsize_dp;  
  ...
  wire   [`i_axi_a2x_2_A2X_IDW-1:0]                        bid;               // AXI PP B Channel 
  wire   [`i_axi_a2x_2_A2X_BRESPW-1:0]                     bresp;
  wire   [A2X_BSBW-1:0]                        bsideband;

  reg    [`i_axi_a2x_2_A2X_IDW-1:0]                        wid;              // Registered Version of last Write ID. 
  ...
  reg   [`i_axi_a2x_2_A2X_IDW-1:0]                         bf_id_r;  
  wire  [A2X_W_PYLD_W-1:0]                     holding_pyld;        // j: Write data holding register

  reg   [7:0]                                  bfcnt;
  wire  [`i_axi_a2x_2_A2X_IDW-1:0]                         bf_id;  
  wire                                         bf_timeout;
  ...
  // HTRANS Decode
  //*************************************************************************************
  assign htrans_nseq    = (hsel & htrans==`i_axi_a2x_2_HTRANS_NSEQ); // j:
  assign htrans_seq     = (hsel & htrans==`i_axi_a2x_2_HTRANS_SEQ);  // j:
  assign htrans_busy    = (hsel & hwrite & htrans==`i_axi_a2x_2_HTRANS_BUSY); // j:
  assign hburst_single  = (hburst==`i_axi_a2x_2_HBURST_SINGLE);
  assign hburst_incr    = (hburst==`i_axi_a2x_2_HBURST_INCR);
  ...
  genvar i;
    for (i = 1; i <A2X_NUM_AHBM; i=i+1) begin : UWID 
      ...
      i_axi_a2x_2_DW_axi_a2x_h2x_w_id
       #(
  ...
  if (A2X_SPLIT_MODE==1) begin: BFS
    always @(posedge clk or negedge resetn) begin: bf_id_PROC
      if (resetn==1'b0) begin
        bf_id_r <= {`i_axi_a2x_2_A2X_IDW{1'b0}};
      end else begin
    ...
    assign bf_id = {`i_axi_a2x_2_A2X_IDW{1'b0}};
  end
  endgenerate
