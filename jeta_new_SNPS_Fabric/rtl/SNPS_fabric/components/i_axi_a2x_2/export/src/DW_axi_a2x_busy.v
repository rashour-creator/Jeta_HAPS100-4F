...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/axi_dev_br/src/DW_axi_a2x_busy.v#12 $ 
**
** --------------------------------------------------------------------
**
...
module i_axi_a2x_2_DW_axi_a2x_busy ( /*AUTOARG*/
  // Outputs
  busy_status,
  // Inputs
 ...
 // This instance design is shared by other module(s) that uses these port(s).
      ...
      i_axi_a2x_2_DW_axi_a2x_bcm21
       #(
         .WIDTH            (1)
        ,.F_SYNC_TYPE      (`i_axi_a2x_2_A2X_SYNC_DEPTH_BUSY)
        ,.VERIF_EN         (`i_axi_a2x_2_A2X_VERIF_EN)
        ,.SVA_TYPE         (1)
...
generate if (A2X_BRESP_MODE==1) begin: BRESP_MODE
  always @(posedge clk_pp or negedge resetn_pp) begin : BUSY_STATUS_1_PROC
    if(resetn_pp==1'b0) begin
      ...
      busy_status   <= bridge_sel | pp_osx_wr | pp_osx_rd | (~aw_fifo_push_empty) | (~snf_fifo_push_empty) | (~w_fifo_push_empty);
    end
  end
end // BRESP_MODE_1
