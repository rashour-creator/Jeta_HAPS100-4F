...
//*************************************************************************************
module i_axi_a2x_2_DW_axi_a2x_h2x_r_ns (/*AUTOARG*/
   // Outputs
 ...
 arvalid, rready, flush, busy,
  ...
  always @(*) begin: nxt_state_PROC
    ...
    case (state)
      ST_AR: begin
        if (hsel && hready && (~hwrite) && (htrans==`i_axi_a2x_2_HTRANS_NSEQ))
          nxt_state = ST_WAIT;
        else if (hsel && hready && (~hwrite) && (htrans==`i_axi_a2x_2_HTRANS_SEQ) && (hburst==`i_axi_a2x_2_HBURST_INCR))
          nxt_state = ST_WAIT;
      end
      ...
      ST_WAIT: begin
        if ((~rflush) && rrvalid && rlast)
          nxt_state =ST_AR;
        else if ((~rflush) && rrvalid && ((~hsel) || (hsel && (htrans==`i_axi_a2x_2_HTRANS_IDLE) || (htrans==`i_axi_a2x_2_HTRANS_NSEQ))))
          nxt_state = ST_AR;
        else if ((~rflush) && rrvalid && (~rlast))
          nxt_state = ST_READ;
      end
     ST_READ: begin
        if (hready && ((~hsel) || (hsel && hwrite) || (hsel && (~hwrite) && (htrans==`i_axi_a2x_2_HTRANS_IDLE))))
          nxt_state = ST_AR;
        else if (hsel && hready && (~hwrite) && (htrans==`i_axi_a2x_2_HTRANS_NSEQ))
          nxt_state = ST_WAIT;
        else if (hsel && hready && (~hwrite) && (htrans==`i_axi_a2x_2_HTRANS_SEQ) && (~rrvalid))
          nxt_state = ST_WAIT;
        else if (hsel && hready && (~hwrite) && (htrans==`i_axi_a2x_2_HTRANS_SEQ) && rlast && rrvalid)
          nxt_state = ST_AR;
      end
      default: begin
        nxt_state = state; 
  ...
  always @(*) begin: rready_PROC
    ...
    if (rflush)
      ...
      rready_r = 1'b1;
    else if ((state==ST_READ) && hsel && (~hwrite) && hready && (htrans==`i_axi_a2x_2_HTRANS_SEQ) && rrvalid)
      rready_r = 1'b1;
  end
  ...
  always @(posedge clk or negedge resetn) begin: arsplit_PROC
    if (resetn == 1'b0) begin
      ...
      if (rrvalid && rlast)
        rflush_r <= 1'b0;
      else if ((state==ST_WAIT) && rrvalid && ((~hsel) || (hsel && ((htrans==`i_axi_a2x_2_HTRANS_NSEQ) || (htrans==`i_axi_a2x_2_HTRANS_IDLE)))))
        rflush_r <= 1'b1;
      else if ((state==ST_READ) && hready && ((~hsel) || (hsel && ((htrans==`i_axi_a2x_2_HTRANS_NSEQ) || (htrans==`i_axi_a2x_2_HTRANS_IDLE)))))
        rflush_r <= 1'b1;
    end
  end
  
  // Needed to stop 1 cycle drop in rready_sp for INCRs who need flushing of extra pre-fetched beats
  wire flush_early = (!rflush_r) && rrvalid && (state==ST_READ) && hready && ((~hsel) || (hsel && ((htrans==`i_axi_a2x_2_HTRANS_NSEQ) || (htrans==`i_axi_a2x_2_HTRANS_IDLE))));
