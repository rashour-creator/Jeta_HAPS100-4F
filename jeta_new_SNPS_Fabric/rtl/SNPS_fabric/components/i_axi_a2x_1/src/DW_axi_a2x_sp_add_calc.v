...
module i_axi_a2x_1_DW_axi_a2x_sp_add_calc (/*AUTOARG*/
  ...
  // Signal Decelaration
  // **************************************************************************************
  wire   [`i_axi_a2x_1_A2X_IDW-1:0]                      id_i;            // Payload
  ...
  wire   [`i_axi_a2x_1_A2X_BTW-1:0]                      burst_o;   
  ...
  always @(*) begin: SM_COMB_PROC
    ...
    case (state)
      ST_WAIT: begin
        if ((A2X_PP_MODE==0) && (A2X_CHANNEL==0) && (buf_mode==`i_axi_a2x_1_SNF_MODE)) begin 
  ...
  // **************************************************************************************
  wire        burst_incr = (burst_i==`i_axi_a2x_1_ABURST_INCR)? 1'b1 : 1'b0; 
  // spyglass disable_block W164b
  // SMD: Identifies assignments in which the LHS width is greater than the RHS width
  // SJ : This is not a functional issue, this is as per the requirement.
