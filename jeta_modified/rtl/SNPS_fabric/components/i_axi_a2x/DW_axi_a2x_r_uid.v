...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/amba_dev/src/DW_axi_a2x_r_uid.v#1 $ 
// --------------------------------------------------------------------
*/
...
module i_axi_a2x_DW_axi_a2x_r_uid (/*AUTOARG*/
  ...
  localparam A2X_URID_PYLD_W             = (A2X_UPSIZE==1)? A2X_RUS_PYLD_W + 1 : (A2X_DOWNSIZE==1)? A2X_RDS_PYLD_W +1 : `A2X_BSW+1;
  ...
  // Signal Decelaration
  // **************************************************************************************
  reg    [`A2X_IDW-1:0]                       uid_r;                 // Unique ID Value
  reg                                         uid_active_r;
  wire                                        rid_match;
  wire                                        aid_match;
  ...
  wire  [`A2X_IDW-1:0]                        rid_sp;    
  wire  [A2X_SP_DW-1:0]                       rdata_sp; 
  wire  [`A2X_RRESPW-1:0]                     rresp_sp; 
  ...
  wire   [`A2X_BSW-1:0]                       r_pp_size;
  wire   [`A2X_BSW-1:0]                       r_sp_ds_size;
  wire   [`A2X_BSW-1:0]                       r_sp_us_size;
  wire   [`A2X_BSW-1:0]                       r_sp_size;
  wire                                        r_last_int;
  ...
  if ((A2X_PP_MODE==1) && (A2X_PP_ENDIAN!=0)) begin: LE_TO_BE8     
    ...
    if (A2X_DOWNSIZE==1) begin
      assign r_pp_size = uid_pyld_o[(`A2X_BSW+A2X_PP_NUM_BYTES_LOG2+2)-1:A2X_PP_NUM_BYTES_LOG2+2];
    end else if (A2X_UPSIZE==1) begin
      assign r_pp_size =  uid_pyld_o[(`A2X_BSW+A2X_SP_NUM_BYTES_LOG2+2)-1:A2X_SP_NUM_BYTES_LOG2+2];
    end else begin
  ...
  if (A2X_SP_ENDIAN!=0) begin: BE8_TO_LE     
    wire   [`A2X_BSW-1:0]                       r_pp_ds_size_w1;
    wire   [`A2X_BSW-1:0]                       r_pp_us_size_w1;
    wire   [`A2X_BSW-1:0]                       r_pp_size_w1;

    if (A2X_DOWNSIZE==1) begin
      assign r_pp_ds_size_w1 = uid_pyld_o[(`A2X_BSW+A2X_PP_NUM_BYTES_LOG2+2)-1:A2X_PP_NUM_BYTES_LOG2+2];
      assign r_sp_size    = (r_pp_ds_size_w1<A2X_SP_MAX_SIZE) ? r_pp_ds_size_w1:A2X_SP_MAX_SIZE;
    end else if (A2X_UPSIZE==1) begin
      assign r_pp_us_size_w1 = uid_pyld_o[(`A2X_BSW+A2X_SP_NUM_BYTES_LOG2+2)-1:A2X_SP_NUM_BYTES_LOG2+2];
