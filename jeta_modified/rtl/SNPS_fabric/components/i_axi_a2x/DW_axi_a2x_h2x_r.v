...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/amba_dev/src/DW_axi_a2x_h2x_r.v#1 $ 
// --------------------------------------------------------------------
*/
...
module i_axi_a2x_DW_axi_a2x_h2x_r (/*AUTOARG*/
  ...
  reg     [`A2X_HBLW-1:0]                  hburst_o_r; 
  wire    [`A2X_HBLW-1:0]                  hburst_o; 
  wire    [`A2X_HBLW-1:0]                  hburst_sel; 
  wire                                     hburst_vld_o;
  ...
  wire                                     rlast;   
                                                         // AXI Read data
  wire  [`A2X_IDW-1:0]                     rid;    
  wire  [`A2X_RRESPW-1:0]                  rresp; 
  wire  [A2X_PP_DW-1:0]                    rdata; 
  wire  [A2X_RSBW-1:0]                     rsideband;  

  wire                                     rlast_lk;   
  wire  [`A2X_IDW-1:0]                     rid_lk;    
  wire  [A2X_PP_DW-1:0]                    rdata_lk; 
  wire  [`A2X_RRESPW-1:0]                  rresp_lk; 
  ...
  if (A2X_LOCKED==1) begin: LK
    ...
    if (A2X_SPLIT_MODE==1) begin
      ...
      assign lk_pyld_valid_w  = ((unlk_seq | unlk_cmp) & hsel & hready & (!hwrite) & (htrans==`HTRANS_NSEQ)) | lk_req;
    end
    ...
    if (A2X_SPLIT_MODE==1) begin: LKSPLIT
      ...
      always @(posedge clk or negedge resetn) begin: bfhready_r_PROC
        if (resetn == 1'b0) begin
          ...
          if (unlk_cmp)
            lk_hready_r <=  1'b1;
          else if (unlk_seq & hsel & hready & (!hwrite) & (htrans==`HTRANS_NSEQ))
            lk_hready_r <=  1'b0;
        end
      end
  ...
  if (A2X_SPLIT_MODE==0) begin: RNS_BLK
      ...
      always @(posedge clk or negedge resetn) begin: lp_pyld_PROC
        if (resetn == 1'b0) begin
          ...
          if ((!lp_mode) && lp_ar_valid && arready) begin
            lp_ar_valid_r <= 1'b0;
            lp_ar_pyld_r  <= {A2X_AR_PYLD_W{1'b0}};
          end else if (lp_mode && hsel & hready & (!hwrite) & (htrans==`HTRANS_NSEQ)) begin               
            lp_ar_valid_r <= 1'b1;
    ...
    assign busy = busy_ns;
      ...
      wire  [`A2X_IDW-1:0]                     r0_rid,        r1_rid,        r2_rid,        r3_rid,       r4_rid;    
      wire  [A2X_PP_DW-1:0]                    r0_rdata,      r1_rdata,      r2_rdata,      r3_rdata,     r4_rdata; 
      wire  [`A2X_RRESPW-1:0]                  r0_resp,       r1_resp,       r2_resp,       r3_resp,      r4_resp; 
      wire  [A2X_RSBW-1:0]                     r0_rsideband,  r1_rsideband,  r2_rsideband,  r3_rsideband, r4_rsideband; 

      wire  [A2X_NUM_AHBM-1:0]                 hburst_vld_bus; 
      wire  [`A2X_HBLW-1:0]                    hburst_bus     [0:A2X_NUM_AHBM-1]; 
      ...
      if (AR_BUF_FULL_EN==1) begin: HB1
        always @(*) begin: burst_PROC
          if (hburst_vld_o) begin
            burst_r       = ((hburst_o==`HBURST_WRAP4) || (hburst_o==`HBURST_WRAP8) || (hburst_o==`HBURST_WRAP16))? `AWRAP: `AINCR;
            hburst_type_r =  (hburst_o==`HBURST_INCR)? 1'b1: 1'b0;
          end else begin
            burst_r       = burst_i;
        ...
        always@(*) begin: arlenPROC
          ...
          case(hburst_sel)
            ...
            `HBURST_WRAP4:  alen_r = 4'h3;
            `HBURST_INCR4:  alen_r = 4'h3;
            `HBURST_WRAP8:  alen_r = 4'h7;
            `HBURST_INCR8:  alen_r = 4'h7;
            `HBURST_WRAP16: alen_r = 4'hf;
            `HBURST_INCR16: alen_r = 4'hf;
            `HBURST_INCR:   alen_r = alen_i;
      ...
      //*************************************************************************************
      assign htrans_nseq    = (htrans==`HTRANS_NSEQ);
      assign htrans_seq     = (htrans==`HTRANS_SEQ);
      assign htrans_idle    = (htrans==`HTRANS_IDLE);
      ...
      assign hburst_bus[0]         = {(`A2X_HBLW){1'b0}};      
      ...
      assign rstat[1] = r1_rid!={`A2X_IDW{1'b0}};
      assign rstat[2] = r2_rid!={`A2X_IDW{1'b0}};
      assign rstat[3] = r3_rid!={`A2X_IDW{1'b0}};
