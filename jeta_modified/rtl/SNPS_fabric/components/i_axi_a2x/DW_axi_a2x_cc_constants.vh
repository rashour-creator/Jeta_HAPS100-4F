...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/amba_dev/src/DW_axi_a2x_cc_constants.vh#1 $ 
**
** --------------------------------------------------------------------
**
...
//==============================================================================
`define __GUARD__DW_AXI_A2X_CC_CONSTANTS__VH__
...
`define A2X_LOWPWR_NOPX_CNT 2


// Name:         A2X_LOWPWR_RST_CNT
// Default:      2
// Values:       0, ..., 15
// Enabled:      (A2X_LOWPWR_IF==1)? 1 : 0
// 
// Defines the number of clock cycles to hold the DW_axi_a2x in reset after a low-power request.
`define A2X_LOWPWR_RST_CNT 2
...
`define A2X_AXI_INTERFACE_TYPE 1


`define A2X_INT_AXI3 0


`define A2X_INT_LTW 1
...
//  - DW_axi_a2x supports only one AHB Master in AHB-Lite mode.  
//  - DW_axi_a2x responds to AHB reads and non-bufferable writes by driving hready low.
`define A2X_AHB_LITE_MODE 0
...
`define A2X_AHB_SPLIT_MODE 1
`define AHB_SPLIT_MODE
...
// Enabled:      ((A2X_PP_MODE==0) && (A2X_AHB_LITE_MODE==0))
// 
// Defines the number of active AHB Masters that the DW_axi_a2x can support. The number of AHB masters specified by this 
// parameter does not include the dummy master; the DW_axi_a2x assumes that the dummy master is HMASTER zero.
`define A2X_NUM_AHBM 8
...
`define A2X_HREADY_LOW_PERIOD 100
...
`define A2X_RS_RATIO 2

`define A2X_RS_RATIO_LOG2 1
...
`define A2X_HCBUF_MODE 1


`define A2X_APB_MODE 0
...
//  - 0: Generates an AHB Read INCR Port for each AHB Master. The DW_axi_a2x uses the hincr_rbcnt corresponding to the AHB 
//  Master for an AHB INCR transaction. 
...
//  - 0: Generates an AHB Write INCR Port for each AHB Master. The DW_axi_a2x uses the hincr_wbcnt corresponding to the 
//  AHB Master for an AHB INCR transaction. 
...
`define A2X_SINGLE_WBCNT 1

// Sets the minimum value of the HINCR length.

`define A2X_HINCR_WBCNT_MIN 0
...
//   
//       A2X_PP_DW/A2X_SP_DW.
`define A2X_HINCR_WBCNT_MAX 0


`define A2X_HINCR_AWLEN_DFLT 0

`define A2X_HINCR_AWLEN_MIN 0
...
// Name:         A2X_NUM_UWID
// Default:      1
// Values:       1, ..., (A2X_PP_MODE==0)? A2X_NUM_AHBM : 16
...
// For more information on usage for this parameter, refer to "Outstanding Transaction Limits" in the DesignWare 
// DW_axi_a2x Databook.
`define A2X_SP_OSAW_LIMIT_P1 3


`define A2X_SP_OSAW_LIMIT 2

`define A2X_OSAW_LIMIT 2
...
// For more information on usage for this parameter, refer to "Outstanding Transaction Limits" in the DesignWare 
// DW_axi_a2x Databook.
`define A2X_PP_OSAW_LIMIT_P1 16


`define A2X_PP_OSAW_LIMIT 15
...
// For more information on usage for this parameter, refer to "Outstanding Transaction Limits" in the DesignWare 
// DW_axi_a2x Databook.
`define A2X_B_OSW_LIMIT_P1 16


`define A2X_B_OSW_LIMIT 15

`define A2X_OSW_LIMIT 15
...
// that must be in the Read Data FIFO before the AHB Master is recalled from 
// Split. 
//  * 2 - 4 data transfers
//  * 3 - 8 data transfers
//  * 4 - 16 data transfers
...
`define A2X_AHB_RRECALL_DEPTH 2
...
//    transaction commences before the AHB Master returns to complete its  
//    transfer. 
//  * AHB EBT Mode(1) - Generates strobed data beats when an AHB Write 
//    transaction is Early Burst Terminated by another Master. When the  
//    AHB Master returns to complete its transfer, it is considered a new write  
//    transaction.

`define A2X_AHB_EBT_MODE 1

// Sets the minimum value of the HINCR length.

`define A2X_HINCR_RBCNT_MIN 0
...
// depth is set to: 
//   
//     (number_of_masters * maximum_Prefetch) 
//   
...
`define A2X_HINCR_RBCNT_MAX 0


`define A2X_LKMODE_MAX_PREFETCH 16


`define A2X_HINCR_ARLEN_DFLT 0

`define A2X_HINCR_ARLEN_MIN 0
...
// For more information on usage for this parameter, refer to "Outstanding Transaction Limits" in the DesignWare 
// DW_axi_a2x Databook.
`define A2X_OSR_LIMIT_P1 16


`define A2X_OSR_LIMIT 15

// Specifies the DW_axi_a2x Address Sideband Bus Width in AHB Mode. When set 
// to 0, the read/write address sideband ports are removed.

`define A2X_HASBW 0


// Name:         A2X_AWSBW
// Default:      0 ((A2X_PP_MODE==0) ? A2X_HASBW : 0)
// Values:       0, ..., 256
...
//               (A2X_PP_MODE==1))
// 
// If true, the primary port master and secondary port slave write and read address channels have QoS signals 
// (awqos_*/arqos_*) in the I/O.
`define A2X_INC_QOS 0
...
//               (A2X_PP_MODE==1))
// 
// If true, the primary port master and secondary port slave write and read address channels have Region signals 
// (awregion_*/arregion_*) in the I/O.
`define A2X_INC_REGION 0


// `define A2X_HAS_REGION
...
`define A2X_AW_FIFO_DEPTH 4

// //reuse-pragma attr CheckExpr[(@A2X_PP_MODE==0) && (@A2X_BRESP_MODE==1) && (@A2X_AHB_SPLIT_MODE==0) && (@A2X_BLW==4)] {@A2X_WD_FIFO_DEPTH<=16}
// //reuse-pragma attr CheckExpr[(@A2X_PP_MODE==0) && (@A2X_BRESP_MODE==1) && (@A2X_AHB_SPLIT_MODE==0) && (@A2X_BLW==5)] {@A2X_WD_FIFO_DEPTH<=32}
// //reuse-pragma attr CheckExpr[(@A2X_PP_MODE==0) && (@A2X_BRESP_MODE==1) && (@A2X_AHB_SPLIT_MODE==0) && (@A2X_BLW==6)] {@A2X_WD_FIFO_DEPTH<=64}
// //reuse-pragma attr CheckExpr[(@A2X_PP_MODE==0) && (@A2X_BRESP_MODE==1) && (@A2X_AHB_SPLIT_MODE==0) && (@A2X_BLW==7)] {@A2X_WD_FIFO_DEPTH<=128}
// //reuse-pragma attr CheckExpr[(@A2X_PP_MODE==0) && (@A2X_BRESP_MODE==1) && (@A2X_AHB_SPLIT_MODE==0) && (@A2X_BLW==8)] {@A2X_WD_FIFO_DEPTH<=256}
...
//   
//      (number_of_AHB_Masters * maximum_burst_length)  
//  
...
// 
// Controls the connection of the A2X_AHB_SPLIT_MODE parmaeter to the Configure Interface Parameter SplitCapable value is 
// set to 1. 
// 
`define A2X_AUTO_LINK_SPLIT_MODE 0
...
`define A2X_AW_SP_PIPELINE 1


`define A2X_RS_AR_TMO 1


`define A2X_RS_AW_TMO 1


`define A2X_RS_R_TMO 0


`define A2X_RS_W_TMO 0


`define A2X_RS_B_TMO 0

//**************************************************************************************************
// Defined Constants (Resized configurations)
//**************************************************************************************************

`define A2X_AHB_LOCKED 0


// `define A2X_HAS_LOCKED


`define A2X_INT_NUM_AHBM 9


`define A2X_UPSIZE 0


`define A2X_DOWNSIZE 1


// `define A2X_IS_UPSIZED

`define A2X_IS_DOWNSIZED


// `define A2X_HAS_RBUF


// `define A2X_IS_EQSIZED

//**************************************************************************************************
// Defined Constants (INCR Enables)
//**************************************************************************************************

`define A2X_HAS_NBUF_MODE


`define A2X_HAS_HINCR_HCBCNT


`define A2X_HAS_SINGLE_RBCNT


`define A2X_HAS_SINGLE_WBCNT


// `define A2X_HAS_CNT_M1


// `define A2X_HAS_CNT_M2


// `define A2X_HAS_CNT_M3


// `define A2X_HAS_CNT_M4


// `define A2X_HAS_CNT_M5


// `define A2X_HAS_CNT_M6


// `define A2X_HAS_CNT_M7


`define A2X_HAS_CNT_M8


// `define A2X_HAS_CNT_M9


// `define A2X_HAS_CNT_M10


// `define A2X_HAS_CNT_M11


// `define A2X_HAS_CNT_M12


// `define A2X_HAS_CNT_M13


// `define A2X_HAS_CNT_M14


// `define A2X_HAS_CNT_M15
...
// `define A2X_HAS_LOWPWR_IF


`define A2X_INT_HASBW 1
...
`define A2X_INT_AWSBW 1

// `define A2X_HAS_WSB

`define A2X_INT_WSBW 1

// `define A2X_HAS_ARSB

`define A2X_INT_ARSBW 1

// `define A2X_HAS_RSB

`define A2X_INT_RSBW 1

// `define A2X_HAS_BSB

`define A2X_INT_BSBW 1

// `define A2X_PP_IS_AHB

`define A2X_PP_IS_AXI

`define A2X_CLK_IS_SYNC

//**************************************************************************************************
// Defined Constants (FIFO Parameter)
//**************************************************************************************************

`define A2X_AW_FIFO_DEPTH_LOG2 2

`define A2X_AR_FIFO_DEPTH_LOG2 2

`define A2X_BRESP_FIFO_DEPTH_LOG2 1

`define A2X_WD_FIFO_DEPTH_LOG2 4

`define A2X_RD_FIFO_DEPTH_LOG2 4

`define A2X_B_OSW_LIMIT_LOG2 4

`define A2X_PP_OSAW_LIMIT_LOG2 4

`define A2X_SP_OSAW_LIMIT_LOG2 1

`define A2X_OSR_LIMIT_LOG2 4

`define A2X_LK_RD_FIFO_DEPTH_LOG2 2

//**************************************************************************************************
// Defined Constants (DW_axi_a2x Parameter)
//**************************************************************************************************

`define A2X_PP_DW_LOG2 6

`define A2X_SP_DW_LOG2 5

`define A2X_PP_NUM_BYTES 8

`define A2X_PP_WSTRB_DW 8

`define A2X_SP_NUM_BYTES 4

`define A2X_SP_WSTRB_DW 4

`define A2X_PP_MAX_SIZE 3

`define A2X_SP_MAX_SIZE 2

`define A2X_MAX_PP_TOTAL_BYTES 2056

`define A2X_MAX_SP_TOTAL_BYTES 1028

`define A2X_MAX_TOTAL_BYTES 2056


`define A2X_MAX_TOTAL_BYTES_LOG2 12

`define A2X_PP_NUM_BYTES_LOG2 3

`define A2X_SP_NUM_BYTES_LOG2 2
...
// `define A2X_PP_DW_128


// `define A2X_PP_DW_256


// `define A2X_PP_DW_512


// `define A2X_PP_DW_1024
...
// `define A2X_SP_DW_128


// `define A2X_SP_DW_256


// `define A2X_SP_DW_512


// `define A2X_SP_DW_1024


`define A2X_DS_RATIO_2


// `define A2X_DS_RATIO_4


// `define A2X_DS_RATIO_8
...
`define A2X_SIM_SEED 200


`define A2X_SIM_APB_CLK_PERIOD 20


`define A2X_SIM_PP_CLK_PERIOD 10


`define A2X_SIM_SP_CLK_PERIOD 10


`define A2X_SP_CLK_SKEW 0


`define A2X_INACTIVE_VAL 0


// `define A2X_ASYNC_CLKS
...
`define A2X_N_TESTCASE_DUPLICATION 0
...
`define A2X_PRIMEPOWER_SIM 0


// `define A2X_SIM_IS_PRIMEPOWER


`define A2X_VERIF_EN 1
