...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/amba_dev/src/DW_axi_a2x_fifo.v#37 $ 
**
** --------------------------------------------------------------------
**
...
module i_axi_a2x_DW_axi_a2x_fifo (
  // Inputs - Push Side 
  clk_push_i,
  resetn_push_i,
      ...
      always @(posedge clk_push_i or negedge resetn_push_i) begin: wcount_PROC
        if (resetn_push_i == 1'b0) begin
          ...
          if (push_rst_n==1'b0) begin
            sclk_wcount <= {LOG2_DEPTH_P1{1'b0}};
          end else if (push_req_n_i ^ pop_req_n_i) begin
            if (!push_req_n_i)
  ...
  //--------------------------------------------------------------------
    ...
    if (DUAL_CLK==0) begin: SCLK_FIFO
      ...
      if (REG_OUT==1) begin: SCLK_REG_OUT
        ...
        i_axi_a2x_DW_axi_a2x_fifo_sclk_r
         #(
...
// This instance design is shared by other module(s) that uses these port(s).
     i_axi_a2x_DW_axi_a2x_bcm65
      ...
      .clk            (clk_push_i),   
      .rst_n          (resetn_push_i),
      .init_n         (push_rst_n), // Synchronous reset, not used.
...
// This instance design is shared by other module(s) that uses these port(s).
      i_axi_a2x_DW_axi_a2x_bcm66
       ...
       .RST_MODE           (0),                // Reset mode, asynch. reset including memory.
       .VERIF_EN           (`A2X_VERIF_EN),    // Verification control enable 
       .MEM_MODE           (0)                 // Mem mode
      ) U_dclk_fifo (
       // Push side - Inputs
       .clk_push        (clk_push_i),
       ...
       .init_push_n     (push_rst_n), // Tied to 1'b1.
       ...
       .init_pop_n      (pop_rst_n), // Never using diagnostic mode.
     ...
     assign push_full_o  = (push_rst_n==1'b0)? 1'b0 : dclk_push_full_o;
     assign push_empty_o = (push_rst_n==1'b0)? 1'b1 : dclk_push_empty_o;
     assign pop_empty_o  = (pop_rst_n==1'b0) ? 1'b1 : dclk_pop_empty_o; 
