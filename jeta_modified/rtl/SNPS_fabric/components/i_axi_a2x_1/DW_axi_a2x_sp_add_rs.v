...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/amba_dev/src/DW_axi_a2x_sp_add_rs.v#1 $ 
// --------------------------------------------------------------------
*/
...
module i_axi_a2x_1_DW_axi_a2x_sp_add_rs (/*AUTOARG*/
  ...
  // Signal Decelaration
  // **************************************************************************************
  wire   [`A2X_IDW-1:0]                      id_i;            // Payload Decode
  ...
  wire   [`A2X_BSW-1:0]                      ds_size;         // Downsized Size
  wire   [`A2X_BSW-1:0]                      us_size;         // Upsized size
  wire   [`A2X_BSW-1:0]                      rs_size;         // Resized Size
  ...
  // spyglass enable_block TA_09
    ...
    assign ds_fixed_decomp = (burst_i==`ABURST_FIXED) && (size_i>A2X_SP_MAX_SIZE);
   // assign ds_fixed_len    = ((pp_bytes_beat-ds_addr_sub[5:0])>>ds_size)-1;
    ...
    assign rs_burst    = ((size_i>A2X_SP_MAX_SIZE) && (burst_i==`ABURST_FIXED))? `ABURST_INCR : burst_i;
  ...
  if (A2X_UPSIZE==1) begin: USBLK
    ...
    // **************************************************************************************
    assign us_size = ((size_i==A2X_PP_MAX_SIZE) && resize_i && (burst_i!=`ABURST_FIXED))? A2X_SP_MAX_SIZE : size_i;
    ...
    always @(*) begin: us_len_PROC
      ...
      if ((size_i==A2X_PP_MAX_SIZE) && resize_i && (burst_i!=`ABURST_FIXED)) begin
