...
//
// Filename    : DW_axi_a2x_bcm21.v
// Revision    : $Id: //dwh/DW_ocb/DW_axi_a2x/amba_dev/src/DW_axi_a2x_bcm21.v#23 $
// Author      : Doug Lee    2/20/05
// Description : DW_axi_a2x_bcm21.v Verilog module for DW_axi_a2x
//
...
module i_axi_a2x_1_DW_axi_a2x_bcm21 (
    clk_d,
    rst_d_n,
    data_s,
    data_d
...
`endif
  ...
  generate
    ...
    if (SVA_TYPE_BIT1BIT0 == 1) begin : GEN_SVATP_EQ_1
      ...
      DW_axi_a2x_sva01 #(WIDTH, (F_SYNC_TYPE & 7), VERIF_EN) P_SYNC_HS (.*);
    end
    ...
    `endif
      ...
      DW_axi_a2x_sva05 #(WIDTH, (F_SYNC_TYPE & 7)) P_SYNC_GC (.*);
    end

    if (SVA_TYPE_BIT2 == 0) begin : GEN_SVATP_BIT2_EQ_0
      DW_axi_a2x_sva08 #(WIDTH) P_SYNC_EQ_RST (.*);
    end
  endgenerate
  `else
    `ifdef DW_MODEL_MISSAMPLES
  DW_axi_a2x_bvm02 U_CLK_DET (
    .clk         (clk_d        ),
    .rst_n       (rst_d_n      ),
    .clk_stopped (clk_d_stopped),
    .clk_period  (clk_d_period )
