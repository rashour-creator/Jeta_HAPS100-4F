...
// Revision: $Id: //dwh/DW_ocb/DW_axi_a2x/amba_dev/src/DW_axi_a2x_sp_add_split.v#1 $ 
// --------------------------------------------------------------------
*/
...
module i_axi_a2x_1_DW_axi_a2x_sp_add_split (/*AUTOARG*/
  ...
  wire   [`A2X_BTW-1:0]                      ws_burst;         // Wrap Splitter output Burst
  ...
  // Burst Type Decode
  assign  wrap     = (burst_i==`ABURST_WRAP)? 1'b1 : 1'b0;

  // AXI Fixed Burst 
  assign  ws_fixed = (A2X_PP_MODE==0)? 1'b0: (burst_i==`ABURST_FIXED)? 1'b1 : 1'b0;
  ...
  if (BYPASS_WS==0) begin: WSBLK
    ...
    // AXI Burst type
    assign ws_burst_w   = (wrap | second_burst)? `ABURST_INCR: burst_i; 
